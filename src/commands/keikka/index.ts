...
export async function runKeikkaSearch(
  ...
  for (const r of rows || []) {
    ...
    seen.set(id, {
      ...
      ownerName: (r.ownerAsiakasNimi as string) ?? null,
    });
  }
 ...
 */
export async function runKeikkaIntakeResolve(
  client: ApiClient,
  body: Record<string, unknown>,
  flags: WriteFlags
): Promise<unknown> {
  return client.post<unknown>("/api/cli/keikka/intake/resolve", body, {
    headers: writeFlagsToHeaders(flags),
    read: true,
  });
}
 ...
 */
export async function runKeikkaIntakeCommit(
  client: ApiClient,
  body: Record<string, unknown>,
  flags: WriteFlags
): Promise<unknown> {
  return client.post<unknown>("/api/cli/keikka/intake/commit", body, {
    headers: writeFlagsToHeaders(flags),
  });
}

export interface KeikkaUpdateFields {
  status?: string;
  vehicle?: number;
  date?: string;
  start?: string;
 ...
 * `flags.dryRun` is set.
 */
export async function runKeikkaCopy(
  client: ApiClient,
  keikkaId: number,
  opts: { date?: string },
  flags: WriteFlags
): Promise<unknown> {
  const creatorPersonId = personIdFromClaims(
    decodeJwtPayload(client.getCurrentToken()),
    "needed as the copy's creatorPersonId"
  );
  const body: Record<string, unknown> = { keikkaId, creatorPersonId };
  if (opts.date) body.newDate = opts.date;

  if (flags.dryRun) {
    return { dryRun: true, wouldCopy: body };
  }
  return client.post<unknown>("/api/keikka/copy", body, {
    headers: writeFlagsToHeaders(flags),
  });
}
...
/** One row of the keikkaTila catalogue, CLI-projected. */
export interface TilaRow {
  tilaId: number;
  name: string | null;
...
/** --full adds the lifecycle wiring and the UI strings behind the tila. */
export interface TilaRowFull extends TilaRow {
  subtitle: string | null;
  description: string | null;
  successTilaId: number | null;
  errorTilaId: number | null;
  iconColor: string | null;
  mainAction: string | null;
  mainAdminAction: string | null;
}
 ...
 */
export async function runKeikkaTilat(
  client: ApiClient,
  opts: { full?: boolean } = {}
): Promise<ListEnvelope<TilaRow | TilaRowFull>> {
  const rows = await client.get<Record<string, unknown>[]>("/api/tila/list");
...
export function registerKeikkaCommands(
  ...
  k.command("tilat")
    ...
    .action(
      jsonAction(getClient, (client, opts: { full?: boolean }) =>
        runKeikkaTilat(client, { full: opts.full })
      )
    );
  ...
  k.command("search [query]")
    ...
    .action(
      guarded(async (query: string | undefined, opts: { search?: string; query?: string; limit?: number; allCompanies?: boolean }) => {
        const client = await getClient();
        const ownerAsiakasId = ownerAsiakasIdFromToken(client, "run `ib auth switch`");
        const result = await runKeikkaSearch(
          client,
  ...
  intakeResolveCmd.action(
    guarded(async (opts: JsonBodyFlags, cmd: Command) => {
      const parsed = resolveJsonBody(cmd, opts, { required: true })!;
      const client = await getClient();
  ...
  addWriteFlagsToCommand(intakeCommitCmd).action(
    guarded(async (opts: WriteFlags & JsonBodyFlags, cmd: Command) => {
      const parsed = resolveJsonBody(cmd, opts, { required: true })!;
      const client = await getClient();
  ...
  addWriteFlagsToCommand(copyCmd).action(
    guarded(async (idStr: string, opts: WriteFlags & { date?: string }) => {
      const client = await getClient();
