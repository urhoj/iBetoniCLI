 ...
 * **announcement board** (the `ilmoitustauluMessages` table behind the Grid
 * sidebar + `/oma` accordion in the web app). One-to-many broadcast: a tenant
 * admin/editor publishes a dated notice (`title` + `body` + `priority`) that
 * every member of the company sees until it expires. This is the
 * announcement-board sibling of `ib message chat` (person-to-person threads)
 * under the `ib message` umbrella — they share only the English word "message".
 *
 * Backend surface (already in prod — this group needs NO backend change; it
 * reuses the existing REST routes, all scoped to the caller's company via the
 * JWT `ownerAsiakasId`, never a client-supplied tenant):
 *   GET    /api/ilmoitustaulu?date=YYYYMMDD   active notices for a day (any member)
 *   GET    /api/ilmoitustaulu/all             every notice incl. expired (admin/editor)
 *   POST   /api/ilmoitustaulu                 create (admin/editor)
 *   PUT    /api/ilmoitustaulu/:messageId      update (admin any · editor own only)
 *   DELETE /api/ilmoitustaulu/:messageId      delete (admin any · editor own only)
 *
 * Auth (server-enforced): reads of the active list are open to any company
 * member; `all` + every write require `isAsiakasAdmin`/`isAsiakasEditor`
 * (full) or `isIlmoitustauluEditor` (own rows only). The CLI adds no new auth
 * surface — a 403 from the backend maps to exit 3.
 */

/** A single announcement-board row (one `ilmoitustauluMessages` record). */
export interface BoardMessage {
  messageId: number;
  ownerAsiakasId?: number;
  title: string;
  body: string;
  /** `info` | `warning` | `urgent` (server defaults to `info`). */
  priority: string;
  /** ISO `YYYY-MM-DD` — the day the notice becomes visible. */
  startDate: string;
  /** ISO `YYYY-MM-DD` or null (null = never expires). */
  expiresAt?: string | null;
  createdAt?: string;
  updatedAt?: string | null;
  /** personId of the author. */
  createdBy?: number;
  [key: string]: unknown;
}

/** Editable fields of a board message (the backend overwrites the whole row on PUT). */
export interface BoardFields {
  ...
  /** ISO `YYYY-MM-DD`. */
  startDate?: string;
  /** ISO `YYYY-MM-DD`, or `null` to clear the expiry. */
  expiresAt?: string | null;
}
 ...
 */
export function toBoardQueryDate(input: string | undefined): string | null {
 ...
 */
export async function runBoardList(
  client: ApiClient,
  dateYyyymmdd: string
 ...
 */
export function buildBoardBody(
  current: BoardMessage | null,
  fields: BoardFields
): BoardFields {
  const base: Partial<BoardMessage> = current ?? {};
  return {
    ...
    expiresAt:
      fields.expiresAt !== undefined ? fields.expiresAt : base.expiresAt ?? null,
  };
}
 ...
 */
export async function runBoardCreate(
  client: ApiClient,
  fields: BoardFields,
  flags: WriteFlags
): Promise<unknown> {
  const proposed = buildBoardBody(null, fields);
  if (flags.dryRun) return { dryRun: true, proposed };
  return client.post<unknown>("/api/ilmoitustaulu", proposed, {
 ...
 */
export async function runBoardDelete(
  client: ApiClient,
  messageId: number,
  flags: WriteFlags
): Promise<unknown> {
 ...
 */
export function registerMessageBoardCommands(
  parent: Command,
  ...
  const b = parent
    ...
    .alias("ilmoitustaulu")
    .description(
      "Company announcement board (ilmoitustaulu) — dated one-to-many notices shown to every member"
    );

  b.command("list")
    .description(
      "List notices ACTIVE on a day (GET /api/ilmoitustaulu). Any company member. " +
    ...
    .action(async (opts: { date?: string }) => {
      ...
      if (!date) {
        failWith(
          `Invalid --date "${opts.date}" — use today|yesterday|tomorrow or YYYYMMDD`,
          4
        );
      }
      try {
  ...
  const createCmd = b
    .command("create")
    .description(
      "Create a notice (POST /api/ilmoitustaulu). --title, --text and --start-date " +
  ...
  const deleteCmd = b
    ...
    .description(
      "Delete a notice (DELETE /api/ilmoitustaulu/:messageId). --reason required. " +
...
const BOARD_ROW =
  "{ messageId, ownerAsiakasId, title, body, priority, startDate, expiresAt, createdAt, updatedAt, createdBy }";
const BOARD_EDIT_PERMS = ["auth.page.ilmoitustaulu.edit", "auth.page.ilmoitustaulu.editOwn"];

export const MESSAGE_BOARD_SPECS: CommandSpec[] = [
  {
    command: "ib message board list",
    description:
      "List notices ACTIVE on a day (startDate ≤ day ≤ expiresAt, or no expiry), newest first. Open to any company member. --date takes today/yesterday/tomorrow or YYYYMMDD (defaults to today). The web UI's per-device dismiss state is NOT applied — the CLI returns the full active set.",
    auth: "any",
    flags: [
      {
        name: "date",
  ...
  },
  {
    ...
    description:
      "List EVERY notice for the company including expired and not-yet-started ones (the admin-panel view). Requires an asiakas admin/editor or ilmoitustaulu editor.",
  ...
  },
  {
    ...
    description:
      "Create a notice (POST). --title, --text and --start-date are required; --priority defaults to info; --expires-at is optional (omit = never expires). Requires admin/editor. --reason required.",
    permissions: BOARD_EDIT_PERMS,
    flags: [
  ...
  },
  {
    ...
    description:
      "Update a notice (PUT). GET-merges the current row (over `all`) so omitted fields are preserved — the backend overwrites the whole row. Admins edit any row; editors only their own. --reason required.",
    permissions: BOARD_EDIT_PERMS,
  ...
  },
  {
    ...
    description:
      "Delete a notice (DELETE, 204). Admins delete any row; editors only their own. --reason required.",
    permissions: BOARD_EDIT_PERMS,
