 ...
 */
export function registerOpendataCommands(
  parent: Command,
  ...
  const od = parent
    .command("opendata")
    .description(
      "Free/open external-data APIs — city building registries, FMI weather, PRH business registry"
    );
  ...
  od.command("prh [ytunnus]")
    .description(
      "Look up a company in the Finnish business registry (PRH) by <ytunnus>, or --search <name>"
    )
