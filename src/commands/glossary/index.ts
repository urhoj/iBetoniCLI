...
export function canonicalGlossarySetJson(json: Record<string, unknown>, keys: Map<string, string>, term?: string): Record<string, unknown> {
  ...
  for (const k of READ_ONLY_ECHO_KEYS) delete json[k];
  if (Array.isArray(json.relatedCommands) && json.relatedCommands.every((r) => r && typeof r === "object" && typeof (r as Record<string, unknown>).command === "string")) {
    json.relatedCommands = (json.relatedCommands as Array<{ command: string }>).map((r) => r.command);
  }
  const unknown = Object.keys(json).filter((k) => !keys.has(k));
  if (unknown.length) {
    const accepted = [...new Set(keys.keys())].filter((k) => !k.includes("-"));
    failWith(
      `--from-json: unknown key${unknown.length > 1 ? "s" : ""} ${unknown.join(", ")} — accepted: ${accepted.join(", ")} (see \`ib glossary set --help\`; a \`glossary lookup\` row round-trips as is)`,
      4
    );
  }
...
export async function runGlossaryDismiss(client: ApiClient, term: string, flags: WriteFlags = {}): Promise<unknown> {
  if (!term.trim()) failWith("dismiss: term must be non-empty", 4);
...
export function registerGlossaryCommands(program: Command, getClient: () => Promise<ApiClient>): void {
  ...
  addWriteFlagsToCommand(dismiss).action(
    jsonAction(getClient, (client, term: string, opts: WriteFlags) =>
      runGlossaryDismiss(client, term, opts)
    ));
  ...
  addWriteFlagsToCommand(del).action(
    jsonAction(getClient, (client, term: string, opts: WriteFlags) =>
      runGlossaryDelete(client, term, opts)
    ));
}
