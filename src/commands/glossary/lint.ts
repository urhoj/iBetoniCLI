 ...
 */
let knownPathCache: { specs: CommandSpec[]; paths: Set<string> } | undefined;
 ...
 */
export function nearDuplicateCandidates(terms: string[]): Array<[number, number]> {
  const buckets = new Map<string, number[]>();
  const add = (key: string, i: number): void => {
    const bucket = buckets.get(key);
    // Terms are keyed in index order, so a repeat of `i` is always the last entry.
    if (!bucket) buckets.set(key, [i]);
    else if (bucket[bucket.length - 1] !== i) bucket.push(i);
  };
  for (let i = 0; i < terms.length; i++) {
    const t = terms[i];
    add(t, i);
    for (let k = 0; k < t.length; k++) add(t.slice(0, k) + t.slice(k + 1), i);
  }
