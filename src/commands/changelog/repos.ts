...
export function normalizeRepoToken(token: unknown): string | null {
  const t = String(token == null ? "" : token).trim();
  if (!t) return null;
  if (/^dbo\./i.test(t)) return t;
  if (/^@ibetoni\//i.test(t)) return t;
  return ALIASES[t.toLowerCase()] || null;
}

/** CSV -> { canonical (deduped CSV), coordinated (subset), unknown (unresolved tokens) }. */
export function normalizeRepoCsv(csv: unknown): {
  canonical: string;
  coordinated: string[];
  unknown: string[];
} {
  const tokens = String(csv == null ? "" : csv).split(",").map((s) => s.trim()).filter(Boolean);
  const canonical: string[] = []; const coordinated: string[] = []; const unknown: string[] = [];
  for (const tok of tokens) {
    const c = normalizeRepoToken(tok);
    if (c == null) { unknown.push(tok); continue; }
    if (!canonical.includes(c)) canonical.push(c);
    if (COORDINATED.includes(c) && !coordinated.includes(c)) coordinated.push(c);
  }
