...
export function registerLegalCommands(
  ...
  legal
    ...
    .action(
      jsonAction(getClient, (client, typeNameArg: string | undefined, opts: { type?: string; meta?: boolean; language?: string }) =>
        runLegalShow(client, resolveTypeNameTarget(typeNameArg, opts.type), !!opts.meta, normalizeLegalLanguage(opts.language))
      )
    );
  ...
  legal
    ...
    .option("--language <l>", LANGUAGE_FLAG_DESC, "fi")
    .action(
      guarded(async (typeNameArg: string | undefined, opts: { type?: string; owner?: number; status?: string; deleted?: boolean; language?: string }) => {
  ...
  legal
    ...
    .option("--type <typeName>")
    .action(
      guarded(async (refArg: string | undefined, opts: { type?: string }) => {
  ...
  for (const [name, run] of [
    ...
    addWriteFlagsToCommand(legal.command(`${name} <documentId>`)).action(
  ...
  legal
    ...
    .action(
      jsonAction(getClient, (client, typeNameArg: string | undefined, opts: { type?: string; docVersion?: string; limit?: number }) =>
        runLegalAcceptances(client, resolveTypeNameTarget(typeNameArg, opts.type), {
          version: opts.docVersion,
          limit: opts.limit,
        })
      )
