...
export function declaredObjectName(definition: unknown): string | null {
  ...
  const m = new RegExp(
    ...
    // and answered "db".
    String.raw`^CREATE\s+(?:OR\s+ALTER\s+)?(?:PROC(?:EDURE)?|FUNCTION|VIEW|TRIGGER)\s+(${SEGMENT}(?:\s*\.\s*${SEGMENT})*)(?!\s*\.)(?!\w)`,
    "i"
  ).exec(s);
  if (!m) return null;
...
export function parseParamLiteral(raw: string): QueryParamValue {
  ...
  if (NUMERIC_LITERAL.test(raw)) {
    ...
    if (!Number.isFinite(n)) {
      failWith(
        `--param value \`${raw}\` overflows to ${n > 0 ? "Infinity" : "-Infinity"}, which serializes as NULL and would silently change the predicate — pass a finite number, or the exact string via --params '{"…":"${raw}"}'`,
        4
      );
    }
    ...
    if (n === 0 && !/^-?0*(?:\.0*)?$/.test(raw)) {
      failWith(
        `--param value \`${raw}\` underflows to 0, which is not the value you wrote — pass a representable number, or the exact string via --params '{"…":"${raw}"}'`,
        4
      );
    }
    ...
    if (n !== 0 && /^-?\d+$/.test(raw) && String(n) !== raw) {
      failWith(
        `--param value \`${raw}\` cannot be represented exactly (it would bind as ${n}) — pass it as a string via --params '{"…":"${raw}"}' if the digits matter`,
        4
      );
    }
    return n;
 ...
 */
function assertBindableNumber(name: string, value: QueryParamValue): void {
  if (typeof value !== "number") return;
  if (!Number.isFinite(value)) {
    failWith(
      `param \`${name}\` is ${Number.isNaN(value) ? "NaN" : "infinite"}, which serializes as NULL and would silently change the predicate — pass a finite number, or the exact digits as a string`,
      4
    );
  }
  if (Number.isInteger(value) && !Number.isSafeInteger(value)) {
    failWith(
      `param \`${name}\` is outside the exactly-representable integer range (it would bind as ${value}) — pass the digits as a string and let SQL Server convert`,
      4
    );
  }
}
...
export function resolveQueryParams(
  ...
  if (json !== undefined) {
    ...
    const fromJson = parsed as Record<string, QueryParamValue>;
    for (const [name, value] of Object.entries(fromJson)) assertBindableNumber(name, value);
    return fromJson;
  }
 ...
 */
export function resolveSqlInput(positional?: string, flag?: string, file?: string): string {
  ...
  if (file !== undefined) {
    ...
    if (!fromFile) {
      failWith(
        file === "-" ? "No SQL arrived on stdin" : `No SQL in ${file} — the file is empty`,
        4
      );
    }
    return fromFile;
...
export function registerSchemaCommands(
  ...
  s.command("query [sql]")
    ...
    .option(
      "--sql-file <file|->",
      "Read the SQL from a file (or - for stdin) instead of argv — shell-safe for multi-line statements and for SQL containing quoted identifiers or string literals, which PowerShell splits on"
    )
    .action(
      jsonAction(
        getClient,
        (client, sql: string | undefined, opts: { sql?: string; sqlFile?: string; param?: string[]; params?: string }) =>
          runSchemaQuery(
            client,
            resolveSqlInput(sql, opts.sql, opts.sqlFile),
            resolveQueryParams(opts.param, opts.params)
          )
      )
    );
