...
export interface SijaintiTypedFields {
  ...
  public?: boolean;
  /**
   * showOnMap — "Näytä kartalla", the /kartta map layer flag. Same tri-state
   * contract as isPublic: ABSENT binds NULL and the save proc's
   * COALESCE(@showOnMap, showOnMap) keeps the stored value, so an unrelated
   * update never flips map visibility (fb#1081).
   */
  showOnMap?: boolean;
}
...
export function buildSijaintiBody(
  ...
  if (typed.showOnMap !== undefined) body.showOnMap = typed.showOnMap;
  return body;
}
...
export function registerSijaintiCommands(
  ...
  addWriteFlagsToCommand(updateCmd).action(
    guarded(async (idStr: string | undefined, opts: WriteFlags & {
      body?: string;
      fromJson?: string;
      id?: number;
      name?: string;
      ...
      if (positionalId !== undefined && opts.id !== undefined && positionalId !== opts.id) {
        failWith(
          `positional sijaintiId (${positionalId}) and --id (${opts.id}) differ — pass only one`,
          4
        );
      }
      const client = await getClient();
