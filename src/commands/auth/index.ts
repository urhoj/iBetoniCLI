...
export function registerAuthCommands(
  ...
  auth
    ...
    .option(
      "--print-token",
      "Print the minted JWT on stdout as JSON ({ token }) for use outside the CLI (curl, browser testing) — e.g. scripting a second endpoint's session without a second interactive login"
    )
    .action(
      guarded(async (opts: { printToken?: boolean }) => {
        try {
          const { token } = await performLogin({
            endpoint: getGlobalOptions(parent).endpoint ?? DEFAULT_ENDPOINT,
            credentialsPath: defaultCredentialsPath(),
          });
          if (opts.printToken) writeJson({ token });
        } catch (e) {
  ...
  auth
    ...
    .option(
      "--dry-run",
      "Preview which session would be revoked and removed, without sending the revoke or touching the credentials file"
    )
    .action(
      guarded(async (opts: { dryRun?: boolean }) => {
        try {
          ...
          if (!creds) {
            ...
            if (opts.dryRun) writeJson({ dryRun: true, wouldRevoke: false, note: "not logged in — nothing to revoke" });
            return;
          }
          ...
          if (borrowed) {
            ...
            if (opts.dryRun) writeJson({ dryRun: true, wouldRevoke: false, note: borrowed });
            return;
          }
    ...
    .action(
      guarded(async () => {
        ...
        if (!resolved) {
          ...
          failWith(
            notLoggedInMessage(endpointOverride, { mentionIbToken: true }),
            2,
            await otherSessionsHint(endpointOverride)
          );
          return;
        }
        const store = createStore(defaultCredentialsPath());
