 ...
 */
const GATED_IGNORED_HINT =
  "⚠ --gated was IGNORED by this backend (ungated rows came back) — " +
  "the filter is deploy-gated and predates the backend serving --endpoint. These results are " +
...
function truncateField(v: unknown): { value: unknown; cut: boolean; elided?: string } {
 ...
 */
function classifyShortened(key: string, fileValue: unknown, currentValue: unknown): string | undefined {
  if (!(TRUNCATED_FIELDS as readonly string[]).includes(key)) return undefined;
  const t = truncateField(currentValue);
  if (!t.cut) return undefined;
  if (fileValue === t.value) return "echo";
 ...
 *
 * `gated` is Omit-ed from the base and redeclared because the two are genuinely
 * different values under one flag name: here it is what Commander parsed (`true`
 * for the bare flag), while on FeedbackFilterParams it is the RESOLVED wire
 * value ("any" or a kind). Widening the base instead would let a raw `true`
 * reach the query string as the literal "true", which the backend rejects.
 */
export interface FeedbackListOptions extends Omit<FeedbackFilterParams, "gated"> {
  unresolved?: boolean;
  all?: boolean;
  full?: boolean;
  mine?: boolean;
...
function warnPartlyShipped(items: Record<string, unknown>[]): void {
  ...
  const isReopened = (r: Record<string, unknown>) =>
    r.status === "open" &&
  ...
  const nameRows = (rows: Record<string, unknown>[]) => {
    const named = rows
  ...
  if (partly.length) {
    warnNote(
      `[ib] note: ${partly.length} of ${active.length} un-closed rows already carry changelog links (${nameRows(partly)}) — ` +
        `part of that work has shipped without closing the row. Read the entry (ib dev changelog get <id>) before claiming one.`
    );
  }
  if (reopened.length) {
    warnNote(
      `[ib] note: ${reopened.length} open row${reopened.length === 1 ? "" : "s"} carry a resolves link — closed once and reopened deliberately (${nameRows(reopened)}). ` +
 ...
 */
export function parseAlsoIds(raw: string | undefined, excludeId: number): number[] | undefined {
...
export async function runFeedbackClaim(
  client: ApiClient,
  id: number,
  input: { by?: string; ttlHours?: number; steal?: boolean; reason?: string; also?: number[] }
): Promise<Record<string, unknown>> {
  ...
  const row = await client.post<Record<string, unknown>>(`/api/feedback/${id}/claim`, body, { headers });
  ...
  if (input.also?.length) {
    ...
    for (const alsoId of alsoIds) {
      try {
        const r = await client.post<Record<string, unknown>>(`/api/feedback/${alsoId}/claim`, body, { headers });
        warnAlreadyLinked(r);
...
export function registerFeedbackCommands(
  ...
  f.command("list")
    ...
    .option("--complexity <n>", "", (v: string) =>
      v.toLowerCase() === SEVERITY_NONE ? SEVERITY_NONE : Number(v)
    )
  ...
  f.command("update <id>")
    ...
    .action(
      guarded(async (
        ...
        applyFromJson(cmd, opts as Record<string, unknown>, UPDATE_FROM_JSON, current);
  ...
  f.command("claim <id>")
    ...
    .action(
      guarded(async (idStr: string, opts: { by?: string; ttlHours?: number; steal?: boolean; reason?: string; also?: string }) => {
