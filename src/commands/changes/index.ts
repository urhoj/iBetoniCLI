/**
 * `ib changes` — changeTracker (audit trail) reading.
 *
...
export async function runChangesEntity(
  client: ApiClient,
  entityType: string,
  entityId: number,
  limit: number,
...
export async function runChangesLatest(
  client: ApiClient,
  limit: number,
...
export async function runChangesRange(
  client: ApiClient,
  opts: {
    from: string;
    to: string;
 ...
 */
export async function runChangesByEntityDate(
  client: ApiClient,
 ...
 */
export async function runChangesUser(
  client: ApiClient,
  personId: number | null,
  limit: number,
  opts: { owner?: number } = {}
    ...
    .action(
      async (
        ...
        try {
          ...
          writeJson(
            await runChangesEntity(client, entityType, Number(entityIdStr), opts.limit, {
              owner: opts.owner,
              field: opts.field,
            })
          );
  ...
  c.command("latest")
    ...
    .action(async (opts: { entityType?: string; owner?: number; limit: number }) => {
      try {
        const client = await getClient();
        writeJson(
          await runChangesLatest(client, opts.limit, {
            entityType: opts.entityType,
            owner: opts.owner,
          })
        );
    ...
    .action(
      async (personIdStr: string | undefined, opts: { owner?: number; limit: number }) => {
        try {
          const client = await getClient();
          writeJson(
            await runChangesUser(client, personIdStr ? Number(personIdStr) : null, opts.limit, {
              owner: opts.owner,
            })
          );
        } catch (e) {
