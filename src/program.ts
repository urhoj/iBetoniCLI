    ...
    guarded(async (
      ...
      if (editOp) {
        ...
        if ((field === "summary" && opts.summary !== undefined) || (field === "detail" && opts.detail !== undefined)) {
          failUsage(
            `edit mode with --field ${field} cannot be combined with --${field} (they target the same field)`
          );
        }
...
export function handleParseRejection(
  ...
  if (isCommanderError(err)) {
    ...
    if (err.exitCode === 0 || err.code === "commander.help") {
      ...
      if (unknownLeaf) return emitUsageEnvelope(err, unknownLeaf, usageEnvelopeResolves(unknownLeaf));
    ...
    if (err.code === "commander.missingMandatoryOptionValue" && erroringCommand) {
      ...
      if (cmd && missing.length) {
        ...
        // Commander checks mandatory options BEFORE unknown ones, so an argv
        // carrying both faults used to report only the missing flag (fb#1179 —
        // see `firstUnknownOption`). Lead with the higher-information fault and
        // fold this envelope's summary into its hint, so both come back at once.
        const unknown = firstUnknownOption(cmd);
        if (unknown) {
          const lead = buildUnknownOptionEnvelope(cmd, unknown, getCallerTier());
          lead.hint = `${lead.hint} Also: ${envelope.error}.`;
          return emitUsageEnvelope(err, lead);
        }
        ...
        if (dateHint) {
          const base = envelope.hint.replace(/[.\s]*$/, "");
