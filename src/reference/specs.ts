...
const BASE_COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    description:
      "Edit a filed row's classification (--scope/--kind/--severity) or its --description (developer-only). The correction twin of `resolve` (which sets status/note) — same PUT /api/feedback/:id endpoint. A real write, blocked under --read-only (exit 3). --dry-run previews the body client-side. Deploy-gated: an older backend ignores these fields and 400s on a status-less body.",
    permissions: ["isSystemAdmin or isDeveloper"],
    tier: "developer",
    mutates: true,
    args: [{ name: "id", type: "number", description: "feedbackId" }],
    ...
    outputShape:
      "A compact ack { feedbackId, scope, kind, severity, updatedAt, description? } (description capped at 200 chars; the full row with --full). With --dry-run: { dryRun:true, wouldSend:{ method, path, body } }.",
    errors: [
