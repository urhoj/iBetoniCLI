...
const BASE_COMMAND_SPECS: CommandSpec[] = [
  ...
  // ─── notification (2) ─────────────────────────────────────────────────────
  {
    ...
    writeFlags: true,
    outputShape:
      "{ success, messageUuid, personId, name, devicesTargeted, successCount, failureCount, activeDeviceCount } | { dryRun:true, wouldSend:{ personId, name, title, body, deviceCount } } (with --dry-run)",
    errors: [
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "related", type: "string", description: 'Comma-separated command paths, e.g. "ib person,ib driver board". Omit to keep; pass "" to clear.' },
