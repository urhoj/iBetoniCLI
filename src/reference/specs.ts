...
export const COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    args: [
      ...
      { name: "entity", type: "string", description: "keikka|vehicle|person|customer|worksite|sijainti|tuote|bug-report|request|offer" },
    ],
    flags: [],
    writeFlags: true,
    outputShape: "{ ok: true, attachmentId, detached: entity } | { dryRun: true, wouldDetach: {...} }",
    errors: [apiErr(403, "Not owner company or missing manager role", "switch to the owner company; need Admin/KeikkaHandler/AttachmentHandler/Owner"), apiErr(404, "Attachment not found", "verify attachmentId"), ...COMMON_AUTH_ERRORS],
    notes: ["Audited via ChangeTracker on the previously-linked entity's timeline.", DEPLOY_NOTE],
    seeAlso: ["ib attachment attach", "ib attachment search"],
    examples: ["ib attachment detach 4711 keikka", "ib attachment detach 4711 bug-report --dry-run"],
  },
  {
  ...
  },
  {
    ...
    description: "Search attachments in the ACTIVE company by file name / comment, or list orphans (no linked entity) with --missing.",
    auth: "any",
    args: [{ name: "text", type: "string", required: false, description: "Search text (optional when --missing)" }],
    flags: [
      { name: "missing", type: "boolean", description: "Only attachments with NO linked entity" },
      { name: "limit", type: "number", default: "100", description: "Max rows (capped at 500)" },
    ],
    outputShape: `ListEnvelope<${ATTACHMENT_ROW}>`,
    errors: [apiErr(400, "Neither text nor --missing given", "pass search text or --missing"), ...COMMON_AUTH_ERRORS],
    notes: ["Tenant comes from the JWT — there is no company parameter.", DEPLOY_NOTE],
    seeAlso: ["ib attachment list", "ib attachment detach"],
    examples: ["ib attachment search kuormakirja", "ib attachment search --missing"],
  },
  // ─── auth (5) ────────────────────────────────────────────────────────────
  {
  ...
  // ─── validate (1) ──────────────────────────────────────────────────────────
  {
    command: "ib validate",
    description:
      "Run a company-setup validation profile — a pass/fail checklist with Finnish details naming what is missing. Profiles: jerry (BetoniJerry provider readiness), betoni (betoni.online customer base setup). Omit the profile (or use 'list') to list available profiles. GET /api/validation/:profile/:asiakasId.",
    ...
    outputShape:
      "No profile: ListEnvelope<{ id, titleFi, description }>. With profile: { profile, asiakasId, asiakasNimi, ok, summary:{required,recommended,optional?}, checks:[{ id, severity:'required'|'recommended'|'optional', status:'pass'|'fail', titleFi, detail? }] }",
    errors: [
      apiErr(401, "Token expired", "ib auth refresh"),
  ...
  // ─── person (3) ──────────────────────────────────────────────────────────
  {
    ...
    description:
      "List the active company's MEMBERS — persons attached to it via asiakasPerson, the same set as `ib customer person list`. `--asiakas N` lists company N's members (you must belong to N); `--owned` lists persons the company OWNS (created, e.g. customer contacts) instead. Optional --role uses ROLE_NAME_BY_TYPEID from @ibetoni/constants.",
    ...
    flags: [
      ...
      },
      {
        ...
        type: "boolean",
        description:
          "List persons the company OWNS (person.ownerAsiakasId — e.g. customer contacts) instead of its members",
      },
      {
        name: "limit",
        type: "number",
    ...
    errors: [
      ...
      apiErr(
        ...
        "you can only list members of companies you belong to (sysadmin/developer bypass)"
      ),
  ...
  },
  {
    ...
    description:
      "Search vehicles by reg-no / name / fleet-number substring (LIKE on vehicleRegNo / vehicleNimi / vehicleNo).",
    permissions: ["auth.page.vehicle.read"],
  ...
  },
  {
    ...
    description:
      "Change-tracker audit trail for one vehicle — who changed which field, when, old→new, with --reason. Alias of `ib log entity vehicle`. For DRIVER-assignment history use `ib vehicle drivers` (different data: personPvm assignments). GET /api/changes/vehicle/:vehicleId/:ownerAsiakasId.",
    auth: "any",
    args: [{ name: "vehicleId", type: "number", description: "vehicleId" }],
    flags: [
      { name: "owner", type: "number", description: "ownerAsiakasId (default: active company)" },
    ...
    seeAlso: ["ib log entity", "ib vehicle drivers"],
  ...
  // ─── sijainti (12) ───────────────────────────────────────────────────────
  {
    ...
    outputShape:
      "ListEnvelope<{ sijaintiId, name, address, coords:{lat,lng}, type, typeName, ownerAsiakasId, ownerName, jerryActiveUntil }> (+truncated:true when the result hit the limit; +hint pointing at --all when 0 rows came back without --all)",
    errors: [
  ...
  },
  {
    ...
    description:
      "List every UI help-text entry (the whole helps table). Useful to discover helpIds before `ib ohje get`/`update`. The HTTP route is unauthenticated, but ib calls it with your session token.",
    auth: "any",
    flags: [
  ...
  },
  {
    ...
    description:
      "All versions of a document type — active, drafts, and superseded history. Content stripped; fetch one version with ib legal get.",
    auth: "any",
    args: [{ name: "typeName", type: "string", description: "Document type name (see ib legal types)" }],
    flags: [
    ...
    outputShape:
      "ListEnvelope<{documentId, version, title, isActive, effectiveDate, createdBy, createdTime, notes, ownerAsiakasId}>",
    errors: COMMON_AUTH_ERRORS,
  ...
  },
  {
    ...
    description:
      "Create a NEW document version (developer/sysadmin only). Versions are IMMUTABLE — any content/title change is a new version; there is no in-place edit. Draft by default (isActive=0); --activate publishes atomically (deactivates prior versions of the same type+tenant). Content from --file (local) or --content (inline; required over /api/cli/exec). --reason required unless --dry-run.",
  ...
  },
  {
    ...
    description:
      "Publish a document version (developer/sysadmin only): atomically deactivates all sibling versions of the same type+tenant and activates this one (also stamps effectiveDate). Use to publish a draft or roll back to an earlier version.",
    permissions: ["isSystemAdmin or isDeveloper (server-enforced)"],
  ...
  },
  {
    ...
    description:
      "Soft-delete (deactivate) a document version (developer/sysadmin only). The row and all acceptance history remain in the database.",
    permissions: ["isSystemAdmin or isDeveloper (server-enforced)"],
