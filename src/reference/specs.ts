 ...
 */
export const COMMON_AUTH_ERRORS: CommandError[] = [
...
const BASE_COMMAND_SPECS: CommandSpec[] = [
  ...
  // ─── sijainti (12) ───────────────────────────────────────────────────────
  {
    ...
    outputShape:
      "ListEnvelope<{ sijaintiId, name, address, coords:{lat,lng}, type, typeName, ownerAsiakasId, ownerName, jerryActiveUntil, maxDeliveryDistance }> (+matchable:boolean on each row when --jerry is set; +truncated:true when the result hit the limit; +hint pointing at --all when 0 rows came back without --all)",
    errors: [
  ...
  },
  {
    ...
    description:
      "List provider-onboarding prospects — pipeline status, tier, outreach contact, live Jerry-active flag, and muistutusDue (email1b reminder due) per company (GET /api/admin/jerry-onboarding). Filters: --status, --tier, --due (client-side on muistutusDue). System-admin only.",
    permissions: ["isSystemAdmin"],
    tier: "developer",
    flags: [
  ...
  },
  {
    ...
    outputShape:
      "{ items: BugReportRow[], nextCursor: null, count, truncated } — each row is enriched client-side with ageDays (whole days since createdAt) and staleDays (since last activity = updatedAt↔createdAt); a high staleDays on an open report flags a months-old untouched bug." +
      TRUNCATED_NOTE,
    errors: [
