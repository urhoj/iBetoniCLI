...
const BASE_COMMAND_SPECS: CommandSpec[] = [
  ...
  // ─── inbox (1) ───────────────────────────────────────────────────────────
  {
    ...
    outputShape:
      "{ generatedAt, needsYou, changelog:{ pending, deployPending, maxBumpLevel }, feedback:{ open, reviewed, byKind:{ open, reviewed } }, bugs:{ new }, support:{ open, truncated }, legal:{ drafts }, glossary:{ misses }, jerry:{ noSupplyLive, noSupplyExpired }, memory:{ feedbackId, entryCount, flaggedForRetire, groomDate, daysSince } | null } — with --details each signal also carries an `items` array (feedback.items splits into { open, reviewed }).",
    errors: [
      apiErr(401, "Token expired", "ib auth refresh"),
