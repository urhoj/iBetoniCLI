...
const BASE_COMMAND_SPECS: CommandSpec[] = [
  ...
  // ─── sijainti (12) ───────────────────────────────────────────────────────
  {
    ...
    outputShape:
      "ListEnvelope<{ sijaintiId, name, address, coords:{lat,lng}, type, typeName, ownerAsiakasId, ownerName, jerryActiveUntil, maxDeliveryDistance }> (+truncated:true when the result hit the limit; +hint pointing at --all when 0 rows came back without --all)",
    errors: [
