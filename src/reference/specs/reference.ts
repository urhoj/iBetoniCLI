...
export const REFERENCE_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "field", type: "string", description: "Edit-mode target field: summary | detail (default detail). Conflicts with --summary/--detail only on the SAME field (fb#1868)." },
      ...
      { name: "reason", type: "string", description: "Audit-log reason (X-Action-Reason); REQUIRED in edit mode (--replace/--append/--prepend), optional for a plain --summary/--detail overwrite" },
    ],
    outputShape: "{ command, runs, … } (backend response) | plain --dry-run: {dryRun:true, wouldSave:{command, exists, writes:{summaryChars?, detailChars?}, aiConfidence, needsHumanReview}, validation} | edit-mode --dry-run: {dryRun:true, command, field, matchCount?, addedLines, removedLines, sameContent, unified, alsoOverwrites?}",
    notes: [
    ...
    errors: [
      ...
      },
      {
        ...
        meaning: "--summary/--detail and the edit op's --field name the SAME field (fb#1868) — a disjoint pair composes fine",
        remedy: "Point --field at the OTHER field, or drop one of the two",
      },
