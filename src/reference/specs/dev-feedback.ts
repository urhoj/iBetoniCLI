...
export const DEV_FEEDBACK_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    errors: [
      { origin: "client", exit: 4, match: ["is required", "must be one of", "must be an integer"], meaning: "Validation", remedy: "description is required; all enums are STRICT — an unknown value exits 4 and is never rewritten: --kind must be improvement|bug|idea|legal, --scope must be cli|app|jerry|bsg2|workspace|security|ops|impeccable|other, --severity (when given) must be critical|major|minor|cosmetic (NOT the issue-tracker vocabulary — high≈major, medium≈minor, low≈cosmetic, blocker≈critical, trivial≈cosmetic; all five are hinted, none is accepted), --gate-kind (when given) must be deploy|soak|legal|owner|backlog; --complexity, when given, must be an integer 1-5. The message names the closest valid value when there is one" },
