...
export const DEV_FEEDBACK_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "gate-ref", type: "string", description: `Gate pointer, meaning depends on --gate-kind: deploy → repo@sha being waited on; legal → TYPE@version (the version being superseded); owner-decision/owner-action → free text naming the call or the task. Max ${FEEDBACK_GATE_REF_MAX} chars. Only meaningful together with --gate-kind.` },
    ...
    errors: [
      ...
      { origin: "client", exit: 4, match: "--gate-ref must be at most", meaning: "--gate-ref exceeds the column width — checked CLIENT-SIDE (fb#1644); the backend's own 400 is Finnish and names neither flag nor limit", remedy: `shorten to ${FEEDBACK_GATE_REF_MAX} chars; long rationale goes in the description` },
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "limit", type: "number", default: "50", description: "Max rows, HARD-CAPPED at 200 by the backend. Asking for more is not an error and not honoured. ANY capped page warns on stderr and sets `truncated: true` — the 50 default counts, not just an over-ask (fb#605, fb#1439)." },
      ...
      { name: "min-age", type: "string", description: "Exclude rows filed within this duration — <n>m|<n>h|<n>d; a bare number exits 4 rather than being guessed at. Drops rows their filer still owns: a just-filed row carries NO claim yet, so --unclaimed cannot tell it from an abandoned one, and newest-first puts exactly those on top. Client-side over the complete walk like --ungated. A row with no parseable createdAt is KEPT (fb#1421)." },
    ],
    outputShape:
    ...
    errors: [
      ...
      { origin: "client", exit: 4, match: ["must be a duration with a unit"], meaning: "--min-age has no unit or is unparseable (client-side, before any request)", remedy: "pass <n>m, <n>h or <n>d, e.g. --min-age 12h — a bare number is refused, not guessed: minutes vs hours is a silent 60x" },
    ...
    notes: [
      ...
      "FRESHNESS NOTE (fb#1421): a stderr note names ACTIVE rows filed in the last 12h; nothing is hidden, and it is suppressed under --min-age. `--unclaimed --ungated --max-complexity 2 --min-age 12h` = the fully-workable batch-fix slice.",
      ...
      "LINKED-BUT-OPEN ROWS: `changelogLinks` on an active row means one of two opposite things; a stderr note names each. `references` = partly shipped (`changelog add --feedback <id> --no-resolve`, fb#647): read the entry (`ib dev changelog get <id>`) BEFORE claiming. `resolves` on an OPEN row = closed once and REOPENED DELIBERATELY (fb#1553; backend-derived, never auto-advanced): its `resolution` says what residue is held open — never close it on the link alone. On a REVIEWED row it is a preserved status (fb#517), not a reopen. Deploy-gated: an older backend sends no links; silence never means 'nothing shipped'.",
    ],
    examples: [
      ...
      "ib dev feedback list --unclaimed --ungated --max-complexity 2 --min-age 12h",
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "gate-ref", type: "string", description: `Gate pointer: deploy repo@sha · legal TYPE@version (the version being superseded) · owner-decision/owner-action free text naming the call or the task. Max ${FEEDBACK_GATE_REF_MAX} chars. ${clearHint("--gate-ref")}` },
