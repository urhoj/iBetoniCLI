...
export const DEV_FEEDBACK_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      "LINKED-BUT-OPEN ROWS: `changelogLinks` on an OPEN/REVIEWED row means one of two opposite things, and a stderr note names each. A `references` link = partly shipped (`ib dev changelog add --feedback <id> --no-resolve`, fb#647): read the entry (`ib dev changelog get <id>`) BEFORE claiming, or you re-investigate shipped work. A `resolves` link = closed once and REOPENED DELIBERATELY (fb#1553; the backend derives this and refuses to auto-advance it): its `resolution` says what residue is held open — never close it on the link alone. Deploy-gated: an older backend sends no links, so silence never means 'nothing shipped'.",
    ],
    examples: [
