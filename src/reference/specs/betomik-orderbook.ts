...
export const BETOMIK_ORDERBOOK_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    errors: [
      ...
      { origin: "client", exit: 4, match: "--status: unknown value", meaning: "--status names a value outside pending|blocked|synced|gone|frozen (incl. removed)", remedy: "Use the listed syncStatus values; read a removed row by id with `ib dev betomik-orderbook row <rowId>`" },
      intParseErr("--limit", "pass a positive integer; it slices the fetched run client-side, so any size works"),
      intParseErr("--offset", "pass an integer >= 0 — the hint on a truncated page names the next one", 0),
  ...
  {
    command: "ib dev betomik-orderbook row",
    description: "One staging row by id, WHATEVER its syncStatus (GET /api/betomik-orderbook/rows/:rowId, fb#1977) — the only read of a row that reached the terminal `removed` (which `rows` excludes, fb#1722). Use it to confirm a re-import reconciled a row away, or to read back a rowId from a `sync-row` response.",
    tier: "developer",
    permissions: [BETOMIK_VIEW_PERMISSION],
    flags: [{ name: "no-raw", type: "boolean", description: "Drop rawJson (the raw sheet cells)" }],
    args: [{ name: "rowId", type: "number", description: "betomikOrderbookImportRowId" }],
    outputShape: "one row, the same shape as a `rows` item (incl. syncStatus, keikkaId, palkkiId, blockReason)",
    errors: [
      { origin: "client", exit: 4, match: "invalid rowId", meaning: "rowId is not a positive integer", remedy: "Pass a numeric betomikOrderbookImportRowId" },
      { http: 404, exit: 5, meaning: "No row with that id for Betomik — or a backend older than fb#1977 (route absent)", remedy: "Check the id against `ib dev betomik-orderbook rows <runId>`; on a pre-fb#1977 backend the route only exists after the next deploy" },
      { http: 403, exit: 3, meaning: "Not a system admin/developer and not an admin of the Betomik company", remedy: "Use a developer token, or an asiakasAdmin of asiakasId 27" },
    ],
    seeAlso: ["ib dev betomik-orderbook rows", "ib dev betomik-orderbook sync-row"],
    examples: ["ib dev betomik-orderbook row 1238", "ib dev betomik-orderbook row 1238 --no-raw --columns syncStatus,keikkaId,palkkiId"],
  },
  {
