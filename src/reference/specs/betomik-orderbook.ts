...
export const BETOMIK_ORDERBOOK_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    description: "Fleet drift for one import run (GET /api/betomik-orderbook/runs/:runId/fleet): every vehicle the sheet ran that week (plate, BETOMIK NRO, RUDUS NRO, maker, boom, PUMPUT/PUMIT table, days) matched to betoni.online, with findings a per-row match cannot see — retired-but-running, changed fleet/Rudus number, boom, maker, grid order (sortNo) off the sheet order (type and name are NOT checked — set once when a truck arrives, ruling 2026-09-20) — plus active vehicles the sheet never runs. The sheet is the fleet truth, so each finding is a change to make in betoni.online (`ib vehicle update`).",
    tier: "developer",
    permissions: [BETOMIK_VIEW_PERMISSION],
    flags: [],
    args: [{ name: "runId", type: "number", description: "importRunId from `runs`" }],
    outputShape: "{ vehicles: [{ plate, betomikNo, rudusNo, maker, boom, table: 'large'|'small', days, rows, vehicleId, name, findings: [{ code: 'missing'|'duplicate'|'retired'|'vehicleNo'|'rudusNo'|'boom'|'maker'|'sortNo'|'ambiguous', message }], expected: { vehicleNo, vehiclePuomi, memo, sortNo } }], pseudo: [{ label, kind: 'extra'|'mixer'|'loan'|'note', days, rows }], notInSheet: [{ vehicleId, name, plate, vehicleNo, vehiclePuomi, lastDate }], summary: { sheetVehicles, withFindings, notInSheet, pseudo }, dayDrivers: [{ plate, cell, days }] }",
    notes: [
      "`expected` is what betoni.online should hold per the sheet (memo `<MAKER>, Rudus NRO <n>`; sortNo = position in sheet order x10, large table first) — vehicles come back in that order, so `ib vehicle update <id> --sort-no <expected.sortNo>` per row reproduces the sheet in the grid. Plate-less pseudo blocks (EXTRA* → Jemma, Betoniauto / Betonirekka → the vehicle of that name, laina kone) are classified, never flagged. Jemma, an already-retired vehicle, and the Betoniauto / Betonirekka in a week their block ran are excluded from notInSheet.",
    ],
    errors: [
      { origin: "client", exit: 4, meaning: "runId is not a positive integer", remedy: "Pass the importRunId from `ib dev betomik-orderbook runs`" },
      { http: 403, exit: 3, meaning: "Not a system admin/developer and not an admin of the Betomik company", remedy: "Use a developer token, or an asiakasAdmin of asiakasId 27" },
  ...
  },
  {
    ...
    description: "Store one run report of the SCHEDULED tick (POST /api/betomik-orderbook/tick-runs). Called by scripts/sales/betomik-sync-tick.sh from its EXIT trap through betomik-tick-report.py — success or failure — so the Tikki admin tab can show when the tick ran, how long it took and what broke. Stored in dbo.log_cronJobs (job betomikOrderbookTick, 30-day retention).",
    tier: "developer",
    auth: "any",
    writeFlags: true,
    dryRunKind: "server",
    ...
    args: [],
    outputShape: "{ stored: true }",
    errors: [
      { http: 400, exit: 4, meaning: "startedAt/finishedAt not ISO, exitCode/isoYear/isoWeek not integers, mode not shadow|create|full, or the body is over 64 KB", remedy: "Fix the report — the reporter builds it from tick.log and sync.out.json" },
      { http: 403, exit: 3, meaning: "Not a system admin or developer", remedy: "Use a developer token" },
    ],
    seeAlso: ["ib dev betomik-orderbook tick-runs"],
    examples: ["ib dev betomik-orderbook tick-report --from-json ~/.betomik-tick/2026-W38/tick-report.json"],
  },
  {
    command: "ib dev betomik-orderbook tick-runs",
    description: "Recent runs of the SCHEDULED tick, newest first (GET /api/betomik-orderbook/tick-runs): start/finish, duration, exit code, failed step, per-step timing, the sync's counts (inserted/updated/unchanged/gone, planned, blocked) and the error tail of a failed run. History is 30 days (log_cronJobs retention).",
    tier: "developer",
    permissions: [BETOMIK_VIEW_PERMISSION],
    ...
    seeAlso: ["ib dev betomik-orderbook tick-report", "ib dev betomik-orderbook runs"],
    examples: ["ib dev betomik-orderbook tick-runs --limit 10"],
  },
];
