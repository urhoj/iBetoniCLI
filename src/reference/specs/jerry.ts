...
export const JERRY_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    outputShape:
      "ListEnvelope<{ pumppuRequestId, status, createdAt, sentAt, expiresAt, customerAsiakasId, customerNimi, operatorName, customerPersonId, customerEmail, customerPhone, osoite, totalM3, kayttokohde, offerCount, acceptedPriceCents, bestPriceCents, sourceChannel }>. Under --provider each row ALSO carries provider: { notifiedAt, viewedAt, viewSource, viewedByPersonId, declinedAt, declineReason, offerStatus, offerPriceCents } — that one company's own fan-out state. `viewSource` is 'authenticated' | 'link' | null and is the field to read, NOT viewedAt: it separates a provider who signed in and opened the lead from somebody who clicked the tokenized link in the notification email, which viewedAt alone conflates (fb#638). customerPersonId/customerEmail/customerPhone are the placing operator's contact info (null when the request has no linked person) — unmasked, like `request offers`, since this route is isSystemAdmin-only (fb#982).",
    errors: [
  ...
  },
  {
    ...
    flags: [],
    outputShape:
      "{ pumppuRequestId, status, createdAt, sentAt, expiresAt, totalM3, kayttokohde, customerAsiakasId, customerNimi, operatorName, customerPersonId, customerEmail, customerPhone, osoite, offerCount, acceptedPriceCents, bestPriceCents, recipients: [{ asiakasId, asiakasNimi, notifiedAt, viewedAt, viewSource, viewedByPersonId, viewedByName, declinedAt, declineReason, declinedByPersonId, declinedByName, declineSource, hasOffer }] }. viewSource: null = never opened · \"authenticated\" = a signed-in person opened it (viewedByPersonId/viewedByName name them) · \"link\" = opened through the tokenized preview link, nobody authenticated. declineSource is its twin over declinedAt/declinedByPersonId, and answers the fb#1268 success question directly: a \"link\" decline came in over the emailed token with no login, which before fb#1268 was impossible (declining was login-gated, and there had never been a single decline). customerPersonId/customerEmail/customerPhone are the placing operator's contact info, unmasked (isSystemAdmin-only route, fb#982) — use to confirm which address the customer-side notification email went to.",
    errors: [
