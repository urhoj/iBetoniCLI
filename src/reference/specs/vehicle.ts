...
export const VEHICLE_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    flags: [
      ...
      { name: "gps-tracking", type: "boolean", description: "Whether the vehicle has GPS tracking hardware installed (hasGpsTracking; gates the GPS keikka-tila row; true/false)" },
    ...
    errors: [
      ...
      numParseErr("--capacity", "pass the capacity in m3 as a number (e.g. 7.5)"),
      numParseErr("--puomi", "pass the boom length in metres as a number"),
      intParseErr("--sort-no", "pass the grid position as an integer"),
      apiErr(400, "Grid CSS-tyyli: …", "--grid-style used a non-cosmetic property or a brace/comment/@-rule/url(); the message names the declaration — keep to background-color, color, border*, outline*, box-shadow, opacity, font-*, text-*, padding*"),
      apiErr(404, "Vehicle not found", "verify vehicleId"),
      ...permErrors("auth.page.vehicle.edit"),
    ],
    examples: [
      ...
      "ib vehicle update 70 --gps-tracking true --reason 'Mapon GPS confirmed'",
    ],
  },
  {
  ...
  },
  {
    ...
    notes: [
      ...
      "`[date]` and `--date` are aliases (today/tomorrow/yesterday or YYYY-MM-DD); omit both and the SERVER resolves today in Europe/Helsinki — unlike `ib vehicle driver …`, omitting the date is not an error here.",
