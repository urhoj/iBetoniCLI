...
export const CUSTOMER_FK_SPECS: CommandSpec[] = [
  sourcesSpec("customer"),
  {
    ...
    description:
      "List a customer's foreign keys (dbo.asiakasForeignKeys) for one owner tenant — the external ids per source (e.g. an accounting-system customer number). Usually one key per source; alias sources such as betomik-orderbook carry one row per spelling.",
    permissions: ["membership of the owner tenant (read)"],
    args: [ASIAKAS_ID_ARG],
    ...
    notes: [OWNER_NOTE],
    seeAlso: ["ib customer fk set", "ib customer fk add", "ib customer fk sources"],
    examples: ["ib customer fk list 1234", "ib customer fk list --asiakas 1234 --owner 8"],
  },
  {
    command: "ib customer fk set",
    description:
      "Set a customer's ONE foreign key on a source, REPLACING any existing key there (server upsert keyed by customer + source + owner) — for a single external id such as an accounting-system number. To keep several keys on one source (order-book name spellings) use `fk add`. The existing rows are read first so the result names the action and an identical key sends nothing. --dry-run resolves client-side (the route has no server dry-run).",
    permissions: ["edit-tier company role on the owner tenant"],
    args: [ASIAKAS_ID_ARG],
    ...
    seeAlso: ["ib customer fk add", "ib customer fk list", "ib customer fk remove"],
  ...
  {
    command: "ib customer fk add",
    description:
      "APPEND one more key on a source, never replacing the existing ones (fb#1975) — how a customer carries several order-book spellings (betomik-orderbook aliases stop the importer creating a duplicate customer). A trimmed, case-insensitive match on the same source is `unchanged` and sends nothing. Keys are unique per owner across ALL sources, so a key held by ANOTHER customer is a 409 naming it. --dry-run resolves client-side.",
    permissions: ["edit-tier company role on the owner tenant"],
    args: [ASIAKAS_ID_ARG],
    flags: [ASIAKAS_TARGET_FLAG, SOURCE_FLAG, KEY_FLAG, OWNER_ASIAKAS_FLAG],
    writeFlags: true,
    dryRunKind: "client",
    outputShape: "{ asiakasId, ownerAsiakasId, source, sourceId, key, action: inserted|unchanged, asiakasForeignKeyId (null on a dry-run insert) } · dry-run: { dryRun:true, would:{ …same } }",
    errors: [
      ASIAKAS_TARGET_ERR,
      OWNER_PARSE_ERR,
      OWNER_UNRESOLVED_ERR,
    ...
    notes: [OWNER_NOTE, "foreignKey is nvarchar(50)."],
    seeAlso: ["ib customer fk import", "ib customer fk list", "ib customer fk remove"],
    examples: [
      "ib customer fk add 1496 --source betomik-orderbook --key 'vepe security' --owner 27 --reason 'sheet spelling'",
      "ib customer fk add 1496 --source betomik-orderbook --key vepe --owner 27 --dry-run",
    ],
  },
  {
    command: "ib customer fk remove",
    description:
      "Delete ONE customer foreign-key row, by <asiakasForeignKeyId> or by --key (trimmed, case-insensitive; --source narrows when the spelling sits on several sources) — so one alias goes without clearing the rest. The row must be on that customer for that owner (checked client-side; the backend delete is also owner-scoped), otherwise exit 5 and nothing is sent. --dry-run resolves client-side.",
    permissions: ["edit-tier company role on the owner tenant"],
    args: [
      { name: "asiakasId", type: "number", description: "customer id" },
      { name: "asiakasForeignKeyId", type: "number", required: false, description: "row id from `ib customer fk list` — or pass --key" },
    ],
    flags: [
      { name: "key", type: "string", description: "Remove the row holding this key instead of naming its id" },
      { name: "source", type: "string", description: "With --key: only match on this source (name or id)" },
      OWNER_ASIAKAS_FLAG,
    ],
    writeFlags: true,
    dryRunKind: "client",
    ...
    seeAlso: ["ib customer fk list", "ib customer fk add"],
    examples: [
      "ib customer fk remove 1234 55 --reason 'wrong number'",
      "ib customer fk remove 1496 --key vepe --source betomik-orderbook --owner 27 --dry-run",
    ],
  },
  {
    command: "ib customer fk import",
    description:
      "Batch `add` from a JSON array (fb#1719) — seed many alias spellings in one pass. Rows: { asiakasId, key, source? }; `source` per row or the --source default. In-file repeats (same customer + source + key) read `unchanged` first; then ONE read per customer and each row appended like `add` (inserted / unchanged). Bad rows, unknown per-row sources and 409s (key on another customer) fail only their own row. --dry-run plans everything and sends nothing. Exit 0 even with failures — read `failed`.",
    permissions: ["edit-tier company role on the owner tenant"],
    args: [{ name: "file", type: "string", description: "JSON array file, or - for stdin (argv-safe for ä/ö)" }],
    flags: [
      { name: "source", type: "string", description: "Default source (name or id) for rows without their own `source`" },
    ...
    examples: [
      ...
      "echo '[{\"asiakasId\":1496,\"key\":\"vepe\"},{\"asiakasId\":1496,\"key\":\"vepe security\"}]' | ib customer fk import - --source betomik-orderbook --owner 27 --dry-run",
    ],
  },
];
