...
export const CUSTOMER_SPECS: CommandSpec[] = [
  ...
  // ─── customer (12) ───────────────────────────────────────────────────────
  {
    ...
    notes: [
      "Scope: regular users see their own tenant + their own company row; SYSTEM ADMINS list across ALL tenants (incl. cross-tenant --ids) — narrow to one tenant's roster with --owner <asiakasId>.",
      ...
      "registeredAt is on every row — combine --since yesterday with --sort registered for a 'new customers in the last 24h' report (cross-tenant for system admins). Both are server-side only (the server truncates at --limit before any client filter could run), so they need the backend deploy.",
    ],
    examples: [
      "ib customer list",
