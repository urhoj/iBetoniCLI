...
export const DOMAIN_BLURBS: Record<string, string> = {
  ...
  palkki: "Day-grid bars (palkit) per vehicle-day — list/get/create/update/delete — their type catalogue (`palkki type`: vehicle-blocking, report visibility, inventory transfer) and their condition-based coloring rules (`palkki color`: ehto/style/icon, distinct from type).",
  stats:
    "Aggregated delivery statistics — `stats --all` is the only CROSS-TENANT (developer) rollup; use it instead of looping `keikka list --company` over companies.",
...
export const FEEDBACK_GUIDANCE = {
  summary:
    "If you are an AI using this CLI, file feedback with `ib dev feedback create` " +
...
export const TOPICS: Topic[] = [
  ...
  },
  {
    ...
    body:
      "0 ok (incl. --help/--version); 1 generic failure: bare `ib`/bare group help render, `auth login` failure, `doctor` aggregate not-ok, unexpected runtime errors; 2 auth (HTTP 401); 3 permission (403, incl. read-only-mode refusals — envelope code READ_ONLY_BLOCKED); 4 validation (4xx incl. 400/409/429, AND usage errors — unknown command/flag, missing required arg/option, whether Commander or a handler standing in for it (e.g. `ib legal show` with no type) rejects them — as the JSON envelope with code USAGE; other local guards exit 4 with code null); 5 not-found (404); 6 server (5xx); 7 network. Every error path emits the JSON envelope on stderr. Each command's --help ERRORS section lists exit code + HTTP status.",
  },
  {
    id: "multi-tenancy",
