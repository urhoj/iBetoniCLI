...
export function assertWritableEndpoint(endpoint: string, forceProd: boolean): void {
  ...
  throw new CliError(
    `Refused: '${endpoint}' is a shared-cache (deployed) endpoint. Cache writes there affect production (prod and staging share one Redis). Re-run with --force-prod to override, or use a local endpoint.`,
    0,
    null,
    3
  );
