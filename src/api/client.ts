...
export function createApiClient({
  ...
  async function request<T = unknown>(
    ...
    if (readOnly && method !== "GET" && !opts.meta && !opts.read) {
      ...
      throw new CliError(
        `Refused: '${method} ${path}' is a write and read-only mode is active (--read-only / IB_READ_ONLY).${dryRunNote}`,
        0,
    ...
    if (res.status === 401 && onRefresh) {
      ...
      try {
        ...
        throw new CliError(
          ...
          null,
          2,
          "session refresh failed — the stored token is expired/invalid and cannot be refreshed; run `ib auth login` to re-authenticate"
        );
      }
