...
export async function performImpersonateExtend(opts: {
  ...
  const data = (await postJson(opts.endpoint, "/api/auth/impersonate/extend", opts.jwt, {}, "Extend")) as {
    token?: string;
  };
  if (!data.token) throw new CliError("Extend failed: response missing token", 0, data, 1);
  return { token: data.token };
}
...
export async function performImpersonateEnd(opts: {
  ...
  const body: Record<string, unknown> = { endReason: "manual" };
  if (opts.sessionId) body.sessionId = opts.sessionId;
  if (opts.targetPersonId) body.targetPersonId = opts.targetPersonId;
...
export function buildImpersonationProfile(
  impToken: string,
  endpoint: string,
