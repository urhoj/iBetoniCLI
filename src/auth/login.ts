...
export async function performLogin(opts: LoginOptions): Promise<void> {
  ...
  try {
    ...
    if (name === "TimeoutError") {
      process.stderr.write(
        "Authorize preflight timed out after 10s — proceeding with the browser flow anyway.\n"
      );
    } else {
      server.close();
      ...
      throw new Error(
        `Cannot reach ${opts.endpoint} (${msg}${cause}) — login cannot succeed from this machine, so the browser was not opened. Check the endpoint/network, or set IB_TOKEN=<jwt> instead.`, { cause: e }
      );
    }
  }
  if (probe && probe.status >= 400) {
    ...
    throw new Error(
      `Authorize preflight failed: HTTP ${probe.status}${detail ? ` — ${detail}` : ""}. The server rejected the authorize URL, so the browser was not opened (no point waiting for a callback).`
    );
  }
