 ...
 */
export function cappedInt(cap: number, flag = "--limit"): (v: string) => number {
  const parse = intFlag(flag, 1);
  return (v: string) => Math.min(parse(v), cap);
}
...
export function parseId(idStr: string, name: string): number {
  ...
  if (!/^\d+$/.test(trimmed) || !Number.isInteger(n) || n <= 0) {
    ...
    if (name === "asiakasId" && trimmed === "0") {
      failWith(
        "asiakasId 0 is the shared (yhteinen) sentinel row, not a customer record — nothing to fetch; its rows surface through their consumers (e.g. `ib betoni laatu list --shared-only`)",
        4,
      );
    }
