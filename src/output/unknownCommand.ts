...
export function descendantsOwningPositional(
  ...
  const owners = COMMAND_SPECS.filter((s) =>
    (s.args ?? []).some((a) => a.name.toLowerCase() === t)
  );
  if (owners.length !== 1) return null;
  const [spec] = owners;
  if (isHiddenAtTier(spec, tier)) return null;
  const base = canonicalPath(group);
  if (group !== "ib" && !spec.command.startsWith(base + " ")) return null;
...
export function buildUnknownCommandEnvelope(
  ...
  const elsewhere =
    curated.length || descendants.length || positional
      ? curated
