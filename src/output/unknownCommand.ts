 ...
 */
const ROOT_FEATURE_DOMAINS: Record<string, SiblingGroup> = {
  grid: {
    domain: "palkki",
 ...
 */
function rootFeatureRedirect(group: string, token: string, tier: CallerTier): SiblingGroupMatch[] {
  if (group !== "ib") return [];
  const feature = ROOT_FEATURE_DOMAINS[token.toLowerCase()];
  if (!feature || fullyHiddenDomains(tier).has(feature.domain)) return [];
 ...
 */
export function descendantsOwningCompoundVerb(
  group: string,
  token: string,
  tier: CallerTier,
  available: string[]
  ...
  if (compoundChildOf(token, available)) return [];
 ...
 */
export function compoundChildOf(token: string, available: string[]): string | null {
  const cut = token.lastIndexOf("-");
  if (cut < 0) return null;
  const prefix = token.slice(0, cut).toLowerCase();
  const verb = token.slice(cut + 1).toLowerCase();
  return (
    available.find((n) => n.toLowerCase() === prefix) ??
 ...
 */
export function descendantsOwningPositional(
  group: string,
  token: string,
  tier: CallerTier
): { path: string; arg: string } | null {
  ...
  if (COMMAND_SPECS.some((s) => s.command.split(" ").pop()!.toLowerCase() === t)) return null;
  ...
  if (isHiddenAtTier(spec, tier)) return null;
  if (group !== "ib" && !spec.command.startsWith(canonicalPath(group) + " ")) return null;
...
export interface UnknownCommandEnvelope {
   ...
   *  GROUP (feedback #343), a descendant subgroup of this group (fb#379), the
   ...
   *  remaining args, e.g. `ib task list`), a descendant whose verb is the token's
   *  last hyphen-segment (fb#1020), or the command that takes the token as a
   *  positional ARGUMENT rather than a verb (fb#1020 — `sql` →
   *  `ib dev schema query`; `hint` spells out that distinction). */
...
export function buildUnknownCommandEnvelope(
  ...
  const descendants =
    curated.length || exactDescendants.length
      ? exactDescendants
  ...
  const elsewhere =
    curated.length || descendants.length || domainRedirect.length
      ? domainRedirect
  ...
  const positional =
    curated.length || descendants.length || elsewhere.length
      ? null
...
export const OPTION_REDIRECTS: Record<string, string> = {
  ...
  "ib dev feedback update --status":
    "`update` edits the row's fields (scope/kind/severity/complexity/description/gate); status is set by `ib dev feedback resolve <id> --status <s>`. To put a closed row back in the queue run `ib dev feedback reopen <id> [note]`.",
};
 ...
 */
export function asDateSuggestion(token: string): string | null {
  const t = token.trim();
  if (/^(today|yesterday|tomorrow)$/i.test(t)) return t.toLowerCase();
  const iso = t.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (iso) return isRealDate(+iso[1], +iso[2], +iso[3]) ? t : null;
...
function dateFlagOf(spec: CommandSpec | undefined): string | null {
 ...
 */
export function optionNamesIn(commands: Iterable<Command>): Set<string> {
  const names = new Set<string>();
  for (const c of commands) {
    for (const o of c.options) {
      if (o.long) names.add(o.long);
      if (o.short) names.add(o.short);
    }
  }
  return names;
}
 ...
 */
export function optionsHoldingFlagName(
  cmd: Command,
  names: ReadonlySet<string>
): Array<{ option: Option; value: string }> {
  const opts = cmd.opts();
  const hits: Array<{ option: Option; value: string }> = [];
  for (const o of cmd.options) {
    const value = opts[o.attributeName()];
    if (typeof value === "string" && names.has(value)) hits.push({ option: o, value });
  }
  return hits;
}
 ...
 */
function eatenEmptyStringFlag(cmd: Command): string | null {
  const chain: Command[] = [];
 ...
 */
function excessCauseHint(cmd: Command): string {
...
export function buildExcessArgumentsEnvelope(
  ...
  if (dated) {
    ...
    if (remedy) parts.push(remedy[0].toUpperCase() + remedy.slice(1).replace(/\.$/, "") + ".");
  ...
  if (availableOptions.length) parts.push(`Accepted flags: ${availableOptions.join(", ")}.`);
 ...
 */
export function truncateOptionToken(token: string): string {
 ...
 */
export function prosePrefixHint(token: string, availableOptions: string[]): string[] {
...
export function buildUnknownOptionEnvelope(
  ...
  const distinctConceptSiblings =
    !fuzzy && guess ? siblingsAcceptingOption(canonical, unknownOption, tier, `--${guess}`) : [];
