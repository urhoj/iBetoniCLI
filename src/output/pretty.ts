 ...
 */
function fitColumns(natural: number[]): number[] | null {
  const n = natural.length;
  const available = availableWidth(n);
  const widths = natural.map((w) => w + 2);
  const sumCapped = (cap: number) =>
    widths.reduce((a, w) => a + Math.min(w, cap), 0);
  if (sumCapped(Number.MAX_SAFE_INTEGER) <= available) return null;
  // largest cap T (≥ minimum) with sum(min(w, T)) <= available
  let lo = MIN_COL_WIDTH;
  let hi = Math.max(...widths);
  while (lo < hi) {
    const mid = Math.ceil((lo + hi) / 2);
    if (sumCapped(mid) <= available) lo = mid;
    else hi = mid - 1;
  }
...
export function renderRecord(record: Record<string, unknown>): string {
  const entries = Object.entries(record).map(
    ([k, v]) => [k, formatCell(v)] as const
  );
  if (entries.length === 0) return chalk.dim("(empty)");
  const natural = [
