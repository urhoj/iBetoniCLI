...
describe("ib message thread run fns", () => {
  beforeEach(() => { mockClient.post.mockReset(); mockClient.patch.mockReset(); mockClient.delete.mockReset(); });

  test("archive POSTs to /archive with reason header", async () => {
    mockClient.post.mockResolvedValueOnce({ threadId: 3, archived: true });
  ...
  test("reopen POSTs to /reopen", async () => {
    mockClient.post.mockResolvedValueOnce({ threadId: 3, archived: false });
  ...
  test("rename PATCHes title", async () => {
    mockClient.patch.mockResolvedValueOnce({ threadId: 3, title: "X" });
  ...
  test("participant add POSTs personId + role", async () => {
    mockClient.post.mockResolvedValueOnce({ added: true });
