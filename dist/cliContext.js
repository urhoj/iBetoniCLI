        ...
        onRefresh: auth.refreshable && !eph.switched && !isImpersonating
            ? endpointMismatch
                ? async () => {
                    ...
                    throw new CliError(`401 from ${endpoint}, but the stored session was minted against ${auth.endpoint} — a token is endpoint-specific (the stored session is likely still valid on its own endpoint)`, 0, null, 2, `authenticate against this endpoint with \`ib auth login --endpoint ${endpoint}\`, or set IB_TOKEN to a token minted for it (local backend: \`node puminet5api/utils/test/mint-local-token.js\`)`);
                }
