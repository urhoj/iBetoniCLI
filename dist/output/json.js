 ...
 */
const isItemEnvelope = (v) => isRow(v) && "item" in v && (v.item === null || isRow(v.item));
/**
...
export function applyColumnsProjection(value, cols) {
    ...
    if (isListEnvelope(value)) {
        ...
        if (value.item === null)
            return value; // nothing found: nothing to project
        rows = [value.item];
    }
    ...
    if (unknown.length)
        warnNote(`[ib] --columns: unknown column(s) ignored: ${listed} — available: ${availableList}`);
    const pick = (r) => {
        const out = {};
        for (const c of cols)
            if (c in r)
    ...
    if (isItemEnvelope(value)) {
        const item = value.item; // narrowed above: non-null here
...
export function writeError(err) {
    ...
    if (err instanceof CliError) {
        ...
        recordFriction(err, undefined, hint ? `${err.message} — ${hint}` : undefined, source === "spec" || source === "error");
