...
export function descendantsOwningVerb(group, token, tier) {
    ...
    if (group === "ib")
        return hits.length <= ROOT_MAX_OWNERS ? hits : [];
 ...
 */
export function topLevelDomainRedirect(group, token, available, tier) {
    const parts = group.split(" ");
    ...
    if (parts.length < 2 || parts[1] === token)
        return [];
    if (!commandDomains(COMMAND_SPECS).includes(token))
        return [];
    if (fullyHiddenDomains(tier).has(token))
        return [];
    if (available.some((name) => name !== token && name.startsWith(token)))
        return [];
    return [
        {
            path: `ib ${token}`,
...
export function buildUnknownCommandEnvelope(cmd, unknownToken, tier) {
    ...
    const livesDeeper = group === "ib" ? "the verb exists in these groups" : "the verb exists deeper in this group";
 ...
 */
export const OPTION_DID_YOU_MEAN_OVERRIDES = {
...
export function siblingsAcceptingOption(command, unknownOption, tier, alsoOwning) {
    ...
    const ranked = (named.length ? named : hits).sort((a, b) => Number(!a.startsWith(parent)) - Number(!b.startsWith(parent)));
 ...
 */
export function siblingsAcceptingSynonym(command, unknownOption, tier) {
 ...
 */
export function writeSafetyIdiomHint(unknownOption, availableOptions, spec) {
    const flag = `--${unknownOption.replace(/^-+/, "")}`;
    if (!WRITE_SAFETY_FLAGS.has(flag))
        return null;
 ...
 */
export function firstUnknownOption(cmd) {
 ...
 */
export function shellSplitHint(token, availableOptions) {
    const flat = token.trim();
    if (/\s/.test(flat))
        return []; // prosePrefixHint owns the prose-block case
    if (/^--?[A-Za-z][\w-]*$/.test(flat))
        return []; // plausibly a flag someone typed
    if (!availableOptions.includes("--from-json"))
        return [];
    return [
...
export function buildUnknownOptionEnvelope(cmd, unknownOption, tier = "developer") {
    ...
    const tenantHint = redirect || didYouMean || bare !== "asiakas"
        ? null
    ...
    if (viaSynonym && acceptedBy.length === 1) {
        ...
        parts.push(`\`${unknownOption}\` is not accepted here or by any sibling; the equivalent \`${viaSynonym.flag}\` is owned by ${acceptedBy.map((c) => `\`${c}\``).join(", ")}.`);
    }
