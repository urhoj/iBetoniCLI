 ...
 */
function enumGuess(value, allowed, synonyms) {
...
export function zeroOneFlag(flag) {
    ...
    return (value) => {
        const n = int(value);
        if (n > 1)
            failWith(`${flag} must be 0 or 1`, 4);
        return n;
    };
}
/**
...
export function resolveTarget(positional, flag, positionalName, flagName) {
    ...
    if (id === undefined || bad(pos) || bad(flag)) {
        failParseUsage(`missing or invalid target: pass <${positionalName}> positionally or via --${flagName} <id>`);
    }
    if (pos !== undefined && flag !== undefined && pos !== flag) {
        failParseUsage(`positional ${positionalName} (${positional}) and --${flagName} (${flag}) differ — pass only one`);
    }
    return id;
}
/**
...
export function resolveDateInput(positional, flag, argName = "date") {
    ...
    const date = pos ?? opt;
    if (date === undefined) {
        failParseUsage(`missing ${argName}: pass <${argName}> positionally or via --${argName} <date> (YYYY-MM-DD, or today/yesterday/tomorrow)`);
    }
    if (pos !== undefined && opt !== undefined && pos !== opt) {
        failParseUsage(`positional ${argName} (${positional}) and --${argName} (${flag}) differ — pass only one`);
    }
    return date;
}
/**
