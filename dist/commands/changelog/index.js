...
const REPO_FLAG_DESC = "Repo(s) this entry ships in (CSV); a cross-lane change names BOTH, e.g. `--repo \"puminet5api,betonicli\"`. THREE outcomes: (1) coordinated — puminet4|puminet5api|puminet7-functions-app|betonijerry|workspace — bumped independently next deploy from the max --bump-level naming it; (2) standalone — betonicli, @ibetoni/*, dbo.*, ibetoni-site, bsg2, betonipumppu — no app bump (version via `npm run final`); (3) any OTHER non-blank value is REJECTED (400/exit 4) at add-time, never reaching a deploy fail-safe. BLANK/omitted --repo differs: it IS accepted, and deploy Step 0 fail-safe-bumps every coordinated repo unless --bump-level none.";
const AREA_FLAG_DESC = "Technical layer: frontend|backend|cli|database|cicd|workspace|site (repo granularity goes in --repo, not here). This is different from `ib dev feedback --scope`, which names the product surface; ops/jerry/security are scopes, not areas.";
...
export async function findAlreadyResolvedFeedback(client, ids) {
    ...
    for (const id of ids) {
        ...
        if (typeof resolver === "number")
            out.push({ feedbackId: id, resolvedByChangelogId: resolver });
    }
    return out;
}
 ...
 */
export function normalizeEnumFlag(value) {
    if (value === undefined)
        return undefined;
 ...
 */
function normalizeChangelogEnums(o) {
    if (o.type !== undefined)
 ...
 * - roundTrip: how `update` treats a read row (fb#1814).
 */
const CHANGELOG_FROM_JSON = {
    ...
    // `update` only (add passes no current row): a read row round-trips (fb#1814).
    roundTrip: {
        volatileKeys: new Set(["updatedAt"]),
        remedies: {
            changelogId: "it names another entry than the positional id — drop the key, or pass the file's id",
            // Loud when EDITED, never folded onto --feedback (fb#576).
...
export function applyFromJson(cmd, o, current) {
    sharedApplyFromJson(cmd, o, CHANGELOG_FROM_JSON, current);
}
/**
...
export function requireAddFields(description, o) {
    ...
    if (![description, o.description, o.summary, o.body].some(has))
        problems.push({
            flag: "--description",
            issue: "missing",
            remedy: "provide the entry text once: positionally, or as --description (aliases: --summary, --body) — copy the envelope's `sample` and fill in real values",
        });
    if (problems.length)
...
export function warnIfPatchIgnored(patch, result, warn = warnNote) {
    ...
    const ignored = DEPLOY_GATED_PATCH_FIELDS.filter((f) => {
        ...
        if (f === "feedbackId" && Array.isArray(row.feedbackLinks)) {
            ...
            const echoed = row.feedbackLinks.map((l) => l?.feedbackId);
 ...
 */
function normalizeFeedbackIds(v) {
    if (v === undefined)
        return undefined;
    const ids = Array.isArray(v) ? v : intCsvFlag("--feedback")(String(v));
 ...
 */
export function warnFeedbackLinkEffects(result, warn = warnNote, { advancesStatus = true } = {}) {
    ...
    for (const l of links) {
        ...
        if (feedbackLinked === false && typeof l.error !== "string")
            warn(`[ib] ⚠ ${at}--feedback named a row that does not exist — cl#${changelogId} was created but NOTHING was linked for it. ` +
        ...
        if (typeof l.error === "string")
            warn(`[ib] ⚠ ${at}the link FAILED (${l.error}). cl#${changelogId} exists and the other ids in this call are reported separately — ` +
        ...
        if (preserved === true || typeof preserved === "string") {
            ...
            warn(`[ib] ⚠ ${at}status advanced to \`applied\`, but the row's EXISTING resolution note was preserved — the two may now contradict each other` +
                (excerpt ? `: "${excerpt}"` : ".") +
 ...
 */
export function excerptNote(text, max = 80) {
 ...
 * nothing.
 */
export function warnFeedbackUnlinkEffects(result, requestedIds, warn = warnNote) {
    ...
    const unlinks = Array.isArray(row.feedbackUnlinks)
        ...
        : null;
    if (!unlinks) {
        warn(`[ib] ⚠ --unlink had NO effect: this backend does not support it yet, and dropped it as an unknown field (the PUT still returned 200). ` +
    ...
    for (const u of unlinks) {
        ...
        if (typeof error === "string")
            ...
            warn(`[ib] note: ${at}there was no link to cl#${changelogId} to remove — nothing changed. ` +
...
export function registerChangelogCommands(parent, getClient, opts = {}) {
    ...
    addWriteFlagsToCommand(c
        ...
        if (o.repo && (o.bumpLevel || "patch") !== "none") {
            ...
            if (coordinated.length === 0 && canonical.length > 0)
                warnNote(`[ib] note: --repo "${o.repo}" is a recognized standalone package, not a coordinated release repo — this entry bumps no app version, so --bump-level ${o.bumpLevel || "patch"} is inert (standalone packages are auto-bumped by \`npm run final\`). This is fine; nothing to do.`);
        }
        if (o.sha)
    ...
    addWriteFlagsToCommand(c
        ...
        // The current row is read only for a file, so a read row round-trips (fb#1814);
        // without a file the client is built where it always was.
        let client;
        let current;
        if (o.fromJson !== undefined) {
            const c = (client = await getClient());
            current = await runWithSiblingHint(c, id, "feedback", () => runChangelogGet(c, id));
        }
        applyFromJson(cmd, o, current);
        normalizeChangelogEnums(o);
        ...
        if (o.appendDescription !== undefined) {
            const row = current ?? (await runChangelogGet(client, id));
        ...
        if (o.feedback !== undefined)
            patch.feedbackId = normalizeFeedbackIds(o.feedback);
        if (o.unlink !== undefined)
            patch.unlinkFeedbackId = normalizeFeedbackIds(o.unlink);
        if (o.resolve === false)
...
export const CHANGELOG_SPECS = [
    {
        ...
        flags: withMaxLengths([
            ...
            },
            {
                name: "no-resolve",
                type: "boolean",
                description: "Record the link(s) with role `references` instead of `resolves`: no status change, an already-resolved row keeps its resolver (fb#548). Applies to EVERY id in the list; for a mixed entry, add the resolving ids here and cross-reference the rest with a second `changelog update … --no-resolve`.",
            },
            {
                name: "take-resolve",
                type: "boolean",
                description: "Force role `resolves` on an ALREADY-resolved row — the deliberate re-own / correction path (fb#366). Without it, such a row links as a cross-reference (fb#880). Conflicts with --no-resolve.",
            },
            {
                name: "sentry",
                type: "string",
    ...
    },
    {
        ...
        flags: [
            ...
            },
            {
                name: "repo",
                type: "string",
                description: "Repo/submodule — matches entries whose --repo CSV CONTAINS this token, so `--repo betonicli` also returns cross-lane rows recorded as \"puminet5api,betonicli\" (deploy-gated: against an older backend this is an exact string match and those rows are invisible). Aliases resolve (`cli` → betonicli); a CSV here matches rows containing ANY of the tokens.",
            },
            {
                name: "feedback",
                type: "number",
    ...
    },
    {
        ...
        flags: withMaxLengths([
            ...
            { name: "title", type: "string", description: "New title" },
            {
                name: "description",
                type: "string",
                description: "New description — REPLACES the whole text (destructive; use --append-description to add to it instead)",
            },
            {
            ...
            },
            {
                name: "no-resolve",
                type: "boolean",
                description: "Record the link(s) with role `references` instead of `resolves`: no status change, an already-resolved row keeps its resolver (fb#548). Applies to EVERY id in the list; for a mixed entry, add the resolving ids first and cross-reference the rest with a second `changelog update … --no-resolve`.",
            },
            {
                name: "sentry",
                type: "string",
            ...
            { name: "language", type: "string", allowed: LANGUAGES, description: "Entry language (fi|en)" },
            {
                ...
                description: "Read the patch CONTENT from a JSON object file (or - for stdin); explicitly-typed flags override. Content keys, in camelCase (description/summary/body, appendDescription, title, type, area, benefits, impact, status, severity, files, repo, sha, commit, vtag, bumpLevel (`bump-level` also accepted), feedback, sentry, source, date, language); files/repo/sha/commit also accept an array of strings. The READ shape is also accepted as input (commitShas→sha, versionTag→vtag, feedbackId→feedback, sentryIssue→sentry, entryDate→date), so a row from `ib dev changelog list` can be edited and posted straight back. " +
                    ...
                    " A `changelog get` row round-trips (fb#1814): unchanged columns are dropped; an edited read-only one exits 4.",
            },
        ]),
        writeFlags: true,
        dryRunKind: "client",
        ...
        errors: [
            ...
            },
            {
                ...
                remedy: "For enums the error names the allowed values (language must be fi|en). For a released entry leave the level as recorded — every other field is still editable; only UNRELEASED entries (ib dev changelog pending) still drive a deploy.",
            },
            {
                origin: "client",
                exit: 4,
                match: "--from-json",
                meaning: "--from-json file is unreadable, not valid JSON, not a JSON object, carries an unknown / wrong-typed key, or (a read row) edits a read-only column",
                remedy: "The error says WHICH: an unopenable path, a JSON syntax error (no field has been read yet, so the key names are not the problem), a root that is not an object, an unknown / wrong-typed key, or a read-row column this command cannot write (the message names the flag or command that writes it — otherwise drop the key)",
            },
            {
                origin: "client",
                exit: 4,
    ...
    },
    {
        ...
        tier: "developer",
        flags: [],
        outputShape: "{ items, entries, maxBumpLevel, count } — `items` is the canonical list key; `entries` is a back-compat alias of the same array (fb#163)",
