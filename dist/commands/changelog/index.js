...
export const CHANGELOG_SPECS = [
    {
        ...
        flags: withMaxLengths([
            ...
            { name: "bump-level", type: "string", default: "patch", allowed: BUMP_LEVELS, description: "App version bump this implies: none|patch|minor|major" },
            {
                name: "feedback",
                type: "string",
                description: "cliFeedback id(s) to link — a single id or CSV (`541,542`), `fb#` anchors accepted. Role `resolves` (default) advances an `open` row to applied; a deliberately-set status (e.g. `reviewed`) is preserved and reported on stderr (fb#517/576). Linking an ALREADY-resolved row TAKES it (the correction path, stderr names the displaced entry; hand-written notes preserved, fb#366).",
            },
            {
                name: "no-resolve",
                type: "boolean",
    ...
    },
    {
        ...
        flags: withMaxLengths([
            ...
            { name: "title", type: "string", description: "New title" },
            {
                name: "description",
                type: "string",
                description: "New description — REPLACES the whole text (there is no append flag; for a partial edit fetch the row with `ib dev changelog get`, edit, resend via --from-json)",
            },
