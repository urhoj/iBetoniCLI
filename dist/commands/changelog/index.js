...
const AREA_SCOPE_REMEDIES = {
    ...
    impeccable: "use --scope impeccable when filing feedback; --area accepts technical layers only",
    jerry: "use --scope jerry when filing feedback; --area accepts technical layers only",
...
const FROM_JSON_NON_PAYLOAD_DESC = "NOT every flag has a JSON twin (fb#631): the BEHAVIOURAL modifiers — --no-resolve (link role) and the write-safety trio --dry-run / --reason / --idempotency-key — stay on the command line, and `noResolve`/`resolve` in the file exit 4.";
...
export function validateEnums(type, area, bumpLevel, source, severity, commandPath = "ib dev changelog add") {
    ...
    if (area !== undefined && !AREAS.includes(area)) {
        ...
        const scope = AREA_SCOPE_REMEDIES[area.toLowerCase()];
        problems.push({
            flag: "--area",
            issue: "invalid",
            got: area,
...
function normalizeFeedbackIds(v) {
    if (v === undefined)
        return undefined;
    return Array.isArray(v) ? v : intCsvFlag("--feedback")(String(v));
}
/**
...
export const CHANGELOG_SPECS = [
    {
        ...
        flags: withMaxLengths([
            ...
            },
            {
                ...
                type: "string",
                requiredGroup: "description input",
                description: "Alias for --description (the entry body); one positional/description/summary/body input is required, and if several are passed they must match",
            },
            {
                name: "body",
                type: "string",
                requiredGroup: "description input",
                description: "Alias for --description (the entry body, free text — NOT the raw-JSON --body of the update commands); one positional/description/summary/body input is required, and if several are passed they must match",
            },
            ...
            { name: "bump-level", type: "string", default: "patch", allowed: BUMP_LEVELS, description: "App version bump this implies: none|patch|minor|major" },
            {
                name: "feedback",
                type: "string",
                description: "cliFeedback id(s) this entry relates to — a single id or a comma-separated list (`541,542,544`), each optionally `fb#`-anchored. Each is recorded in devChangelogFeedback with role `resolves` (default) or `references` (--no-resolve), and a `resolves` link advances the row to applied. The status only advances FROM `open`: a deliberately-set status (`reviewed` = draft awaiting activation) is preserved and reported on stderr (fb#517), and so is a deliberately REOPENED `open` row (fb#576). If a row was ALREADY resolved by another entry, a `resolves` link TAKES it (the correction path) and says so on stderr; any hand-written resolution note is preserved (fb#366).",
            },
            {
                name: "no-resolve",
                type: "boolean",
                description: "Record the link(s) with role `references` instead of `resolves`: no status change, and an already-resolved row keeps its resolver (fb#548). Applies to EVERY id in the list — the flag describes the ENTRY (\"this records staged work\"), not individual rows. For a mixed entry, add the resolving ids first and cross-reference the rest with a second `changelog update ... --no-resolve`.",
            },
            {
                name: "sentry",
                type: "string",
    ...
    },
    {
        ...
        flags: withMaxLengths([
            ...
            { name: "language", type: "string", allowed: LANGUAGES, description: "Entry language (fi|en)" },
            {
                name: "from-json",
                type: "string",
                description: "Read the patch CONTENT from a JSON object file (or - for stdin); explicitly-typed flags override. Content keys, in camelCase (description/summary/body, title, type, area, benefits, impact, status, severity, files, repo, sha, commit, vtag, bumpLevel (`bump-level` also accepted), feedback, sentry, source, date, language); files/repo/sha/commit also accept an array of strings. The READ shape is also accepted as input (commitShas→sha, versionTag→vtag, feedbackId→feedback, sentryIssue→sentry, entryDate→date), so a row from `ib dev changelog list` can be edited and posted straight back. " +
                    FROM_JSON_NON_PAYLOAD_DESC +
