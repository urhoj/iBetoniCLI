 ...
 */
export async function runVehicleCreate(client, fields, flags, opts = {}) {
    ...
    if (fields.vehicleRegNo && !opts.force) {
        ...
        const conflict = existing.items.find((v) => typeof v.plate === "string" &&
            v.plate.toUpperCase() === plate &&
            v.lastDate == null &&
            v.deletedTime == null);
        if (conflict) {
            failWith(`An active vehicle with plate ${fields.vehicleRegNo} already exists under asiakasId ${targetAsiakasId} (vehicleId ${conflict.vehicleId}) — pass --force to create a duplicate anyway.`, 4);
        }
    }
    if (flags.dryRun) {
...
export function registerVehicleCommands(parent, getClient) {
    ...
    const createCmd = addVehicleFieldFlags(v.command("create"), "create").option("--force", "Create even if an active vehicle with the same --reg plate already exists under the target tenant");
    addWriteFlagsToCommand(createCmd).action(guarded(async (opts) => {
        const result = await runVehicleCreate(await getClient(), vehicleFieldsFromOpts(opts), opts, { force: opts.force });
        writeJson(result);
    }));
