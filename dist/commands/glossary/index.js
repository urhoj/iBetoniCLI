 ...
 */
export function glossarySetJsonKeys(set) {
...
export function canonicalGlossarySetJson(json, keys) {
    ...
    for (const [k, v] of Object.entries(json))
        if (v === null && NULL_CLEARS.has(keys.get(k)))
            out[keys.get(k)] = null;
    return out;
}
/**
...
export function mergeSetInput(json, flags) {
    return {
        ...
        appendDefinition: flags.appendDefinition ?? json.appendDefinition,
    };
}
 ...
 */
export async function runGlossaryImport(client, entries, flags, keys) {
    const results = new Array(entries.length);
    let next = 0;
    await Promise.all(Array.from({ length: Math.min(IMPORT_CONCURRENCY, entries.length) }, async () => {
        while (next < entries.length) {
            const i = next++;
            const e = entries[i];
            const { term: rawTerm, ...rest } = e;
            const term = rawTerm ?? null;
            if (!term) {
                results[i] = { term: null, ok: false, error: "missing term" };
                continue;
            }
            try {
                ...
                await runGlossarySet(client, term, { ...inp, updateOnly: flags.updateOnly }, flags);
...
export async function runGlossaryDelete(client, term, flags = {}) {
    if (flags.dryRun) {
        ...
        try {
            const res = await client.get(`/api/cli/glossary${qs({ search: term })}`);
...
export function registerGlossaryCommands(program, getClient) {
    ...
    addWriteFlagsToCommand(imp)
        ...
        writeJson(await runGlossaryImport(await getClient(), arr, opts, glossarySetJsonKeys(set)));
    }));
    const del = glossary
        .command("delete")
        .argument("<term>", "Canonical term");
