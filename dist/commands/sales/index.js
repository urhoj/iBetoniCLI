...
export async function runProspectList(client, opts = {}) {
    ...
    if (opts.search) {
        ...
        // Matches the UI's joined string (companyName + " " + asiakasNimi,
        // salesProspectFilters.js) so a needle straddling that space boundary
        // matches here too (fb#920) — this subsumes matching either field alone,
        // so only this command's own extras (ytunnus + region, fb#817) need a
        // separate check.
        all = all.filter((r) => {
            const joined = `${r.companyName || ""} ${r.asiakasNimi || ""}`.toLowerCase();
            return (joined.includes(needle) ||
                [r.ytunnus, r.region].some((v) => String(v ?? "").toLowerCase().includes(needle)));
        });
    }
    if (opts.brief) {
        all = all.map((r) => {
