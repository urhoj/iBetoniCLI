...
export function registerAuthCommands(parent, isReadOnly) {
    ...
    auth
        ...
        if (resolved.source === "file") {
            out.sessions = (await store.sessions()).map(({ endpoint, personId, ownerAsiakasId, ownerAsiakasName, expiresAt, active }) => ({
    ...
    auth
        ...
        try {
            ...
            if (creds.impersonation) {
                failWith("refresh is disabled while impersonating (it would escalate to a permanent login as the target)", 4, "use `ib auth impersonate --extend` for 10 more minutes, or `--end` to restore your own login");
            }
    ...
    auth
        ...
        .option("--end")
        .option("--extend")
        .action(guarded(async (personId, opts) => {
        const store = createStore(defaultCredentialsPath());
        ...
        await store.save(buildImpersonationProfile(token, impEndpoint, decoded, new Date().toISOString()), undefined, { activate: true });
        writeJson({
            ok: true,
            impersonating: {
