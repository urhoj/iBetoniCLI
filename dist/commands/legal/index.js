...
export function resolveTypeNameTarget(positional, flag, label = "typeName") {
    ...
    if (!name) {
        throw new CliError(`missing document type: pass <${label}> positionally or via --type <typeName>`, 0, { code: "USAGE" }, 4);
    }
    if (positional !== undefined && flag !== undefined && positional !== flag) {
        throw new CliError(`positional ${label} (${positional}) and --type (${flag}) differ — pass only one`, 0, { code: "USAGE" }, 4);
    }
    return name;
}
