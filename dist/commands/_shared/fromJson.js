 ...
 */
function dropUnchangedEchoes(json, keys, cfg, current) {
    const rt = cfg.roundTrip;
    if (!rt)
        return json;
    const out = { ...json };
    for (const k of rt.volatileKeys)
        delete out[k];
    const problems = [];
    for (const k of Object.keys(out)) {
        if (!(k in current))
            continue;
        const verdict = JSON.stringify(out[k]) === JSON.stringify(current[k]) ? "echo" : rt.classify?.(k, out[k], current[k]);
        if (verdict === "echo") {
            delete out[k];
        }
        else if (verdict) {
    ...
    if (problems.length)
        failUsage(`${cfg.flagName ?? "--from-json"}: ${problems.join("; ")}`);
    return out;
}
/**
...
export function normalizeFromJson(json, keys, cfg = {}) {
    ...
    for (const [rawKey, value] of Object.entries(json)) {
        ...
        if (csv.has(key) && Array.isArray(value)) {
            ...
            const isAcceptable = (v) => typeof v === "string" || (tolerant && typeof v === "number" && Number.isFinite(v));
            if (!value.every(isAcceptable))
                problems.push(`"${rawKey}" array must contain only strings${tolerant ? " or numbers" : ""}`);
            else
                out[key] = value.map((v) => String(v).trim()).filter(Boolean).join(",");
            continue;
        }
