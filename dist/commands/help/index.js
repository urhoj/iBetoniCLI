...
export function runHelpTopic(id) {
    ...
    const g = GLOSSARY.find((x) => x.term.split("/").some((alias) => alias.trim().toLowerCase() === needle));
    if (g)
        return { id, title: `${g.term} (glossary)`, body: g.definition };
    const ids = TOPICS.map((x) => x.id).join(", ");
    const terms = GLOSSARY.map((x) => x.term).join(", ");
    throw new CliError(`unknown topic: ${id}. Valid topics: ${ids}. Glossary terms (also resolvable here): ${terms}`, 0, null, 5);
}
/**
