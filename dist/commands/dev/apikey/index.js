...
export async function runApikeySources(client) {
    const res = await client.get("/api/cli/apikeys/sources");
    return listEnvelope(res.items);
}
/** GET /api/cli/apikeys/list — every key configured for a tenant, across all sources. */
export async function runApikeyList(client, ownerAsiakasId) {
    const res = await client.get(`/api/cli/apikeys/list${qs({ ownerAsiakasId })}`);
    return listEnvelope(res.items);
}
/** GET /api/cli/apikeys/verify — one named key, or every key under a source when --name is omitted. */
export async function runApikeyVerify(client, input) {
    if (!input.asiakas)
        failWith("--asiakas is required", 4);
    if (!input.source)
 ...
 */
export async function runApikeySet(client, input, flags) {
    if (!input.asiakas)
        failWith("--asiakas is required", 4);
    if (!input.source)
        failWith("--source is required", 4);
    if (!input.name?.trim())
        failWith("--name is required", 4);
    if (input.value && input.valueStdin)
        failWith("--value and --value-stdin are mutually exclusive", 4);
    const value = input.valueStdin ? readValueFromStdin() : input.value;
    if (!value) {
        failWith("Provide the credential via --value <text> or --value-stdin (stdin avoids the value landing in shell history)", 4);
    }
    const body = {
    ...
    if (input.description)
        body.apiKeyDescription = input.description;
    if (input.expires)
        body.expires = resolveDate(input.expires);
    return client.post("/api/cli/apikeys/set", body, {
        headers: writeFlagsToHeaders(flags),
    });
}
/** DELETE /api/cli/apikeys/revoke — soft revoke (apiKeyActive=0); idempotent. */
export async function runApikeyRevoke(client, input, flags) {
    if (!input.asiakas)
        failWith("--asiakas is required", 4);
    if (!input.source)
        failWith("--source is required", 4);
    if (!input.name?.trim())
        failWith("--name is required", 4);
    return client.delete(`/api/cli/apikeys/revoke${qs({
        ownerAsiakasId: input.asiakas,
        apiKeySourceId: input.source,
        apiKeyName: input.name.trim(),
    })}`, { headers: writeFlagsToHeaders(flags) });
}
export function registerApikeyCommands(parent, getClient) {
    const cmd = parent
        .command("apikey")
        .description("Sysadmin credential admin for dbo.apiKeys (Ecofleet/Fennoa/Mapon/SendGrid/Netvisor/...) — create/replace, revoke, verify, list. The credential value never appears in any response.");
    ...
    cmd
        ...
        if (!opts.asiakas)
            failWith("--asiakas is required", 4);
        return runApikeyList(client, opts.asiakas);
    }));
    cmd
        .command("verify")
        .option("--asiakas <id>", "Target tenant ownerAsiakasId", intFlag("--asiakas"))
        ...
        .action(jsonAction(getClient, (client, opts) => runApikeyVerify(client, opts)));
    ...
    addWriteFlagsToCommand(cmd
        ...
        .option("--asiakas <id>", "Target tenant ownerAsiakasId", intFlag("--asiakas"))
        .option("--source <id>", "apiKeySourceId (see `ib dev apikey sources`)", intFlag("--source"))
        .option("--name <name>", "apiKeyName to revoke")).action(jsonAction(getClient, (client, opts) => runApikeyRevoke(client, opts, opts)));
}
