...
export function registerSchemaCommands(parent, getClient) {
    ...
    s.command("table <name>").description("Columns, keys, FKs, and indexes for a table").action(runOne(runSchemaTable));
    s.command("view <name>").description("Columns and definition (T-SQL) for a view").action(runOne(runSchemaView));
    s.command("proc <name>")
