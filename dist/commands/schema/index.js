 ...
 */
export function declaredObjectName(definition) {
    ...
    for (;;) {
        ...
        if (s.startsWith("/*")) {
            const end = s.indexOf("*/");
            if (end === -1)
                return null; // unterminated — nothing safe to classify
            s = s.slice(end + 2);
            continue;
        }
        if (s === before)
...
export async function runSchemaView(client, name) {
    return withRenameNote(await client.get(`/api/cli/schema/view/${name}`));
}
export async function runSchemaProc(client, name) {
    return withRenameNote(await client.get(`/api/cli/schema/proc/${name}`));
}
export async function runSchemaTrigger(client, name) {
    return withRenameNote(await client.get(`/api/cli/schema/trigger/${name}`));
}
 ...
 */
const METADATA_FILTERED_CATALOGS = /\b(?:sys\.(?:procedures|objects|all_objects|sql_modules|parameters)|information_schema\.(?:routines|parameters)|(?:OBJECT_DEFINITION|OBJECTPROPERTY(?:EX)?)\s*\()/i;
/**
 ...
 */
function catalogFilterHint(sql) {
...
export function resolveSqlInput(positional, flag, file) {
    ...
    const inline = foldAliases([positional, flag], "Provide the SQL once — via the positional or --sql; if both are given they must match");
    if (file !== undefined) {
        if (inline) {
 ...
 */
const OBJECT_NAME_DID_YOU_MEAN_OVERRIDES = {
    palkki: "grid_palkit",
};
/**
...
async function nearestObjectNameSuggestion(client, badName) {
    ...
    const match = overrideTarget && names.includes(overrideTarget) ? overrideTarget : closestName(bare, names, {});
 ...
 */
export function queryTables(sql) {
    const out = [];
    ...
    for (const m of code.matchAll(re)) {
        ...
        if (!out.some((t) => t.table.toLowerCase() === m[1].toLowerCase()))
 ...
 */
async function columnNameSuggestion(client, sql, badColumn) {
    ...
    tables.slice(0, 6).map(async ({ table }) => {
        ...
        return { table, columns: cols.map((c) => c.name).filter((n) => typeof n === "string") };
    }));
    ...
    for (const { table, columns } of known) {
        ...
        if (match)
            return `did you mean ${table}.${match}? (nearest column in ${known.map((t) => t.table).join(", ")})`;
    }
    const listing = known
        .map(({ table, columns }) => {
