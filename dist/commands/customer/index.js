 ...
 */
export function projectCustomerRow(row, fields, sijaintiTypes) {
    let out = row;
    if (sijaintiTypes && sijaintiTypes.length > 0 && Array.isArray(row.sijainnit)) {
        const allowed = new Set(sijaintiTypes);
        out = {
            ...row,
            sijainnit: row.sijainnit.filter((s) => allowed.has(Number(s.sijaintiTypeId))),
        };
    }
    if (fields && fields.length > 0) {
        const projected = { asiakasId: out.asiakasId };
        for (const f of fields) {
            if (f !== "asiakasId" && f in out)
                projected[f] = out[f];
        }
        // Include-arrays are governed by --include, not --fields — keep them.
        if ("contacts" in out)
            projected.contacts = out.contacts;
        if ("sijainnit" in out)
            projected.sijainnit = out.sijainnit;
        out = projected;
    }
    return out;
}
...
export async function runCustomerUpsert(client, opts, flags) {
    ...
    const created = await reconcileCreatedExtras(client, await runCustomerGet(client, newId), { email: opts.email, comment: opts.comment }, flags);
 ...
 */
function parseChanges(setCsv, unsetCsv, valid, renderValid) {
...
export function buildAsiakasCreateBody(flags, ownerAsiakasId, prh) {
    ...
    if (prh)
        applyPrhFlags(body, prh, "yTunnus");
 ...
 */
function applyPrhFlags(body, prh, ytunnusKey) {
    if (prh.name && prh.name !== "Unknown")
        body.asiakasNimi = prh.name;
    if (prh.businessId)
        body[ytunnusKey] = prh.businessId;
 ...
 */
async function reconcileCreatedExtras(client, current, desired, flags) {
    if (flags.dryRun)
        return current;
    const patch = {};
    ...
    if (patch.email === undefined && patch.comment === undefined)
        return current;
...
export function registerCustomerCommands(parent, getClient) {
    ...
    addWriteFlagsToCommand(updateCmd).action(guarded(async (idStr, opts) => {
        ...
        const [current, prh] = await bothInOrder(runCustomerGet(client, asiakasId), opts.fromPrh ? runCustomerPrhById(client, opts.fromPrh) : Promise.resolve(undefined));
