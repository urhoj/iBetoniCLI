...
export function registerSijaintiCommands(parent, getClient) {
    ...
    const createCmd = s
        .command("create")
        .description("Create a new sijainti (POST /api/geocode/sijainti/add). Use typed flags or --body JSON (typed flags win).")
    ...
    addWriteFlagsToCommand(createCmd).action(async (opts) => {
        try {
            ...
            if (Object.keys(body).length === 0) {
                writeError(new Error("provide --body or at least one field flag (--name/--address/--type/--lat/--lng)"));
                process.exit(4);
            }
