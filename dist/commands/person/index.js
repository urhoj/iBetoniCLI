...
export async function runPersonRoleRevoke(client, personId, asiakasId, roleTypeId, flags) {
    ...
    return await client.delete(`/api/asiakasPersonSettings/byRole/${asiakasId}/${personId}/${roleTypeId}`, { headers: writeFlagsToHeaders(flags) });
}
/**
...
export function registerPersonCommands(parent, getClient, getClientForAsiakas) {
    ...
    addWriteFlagsToCommand(notifyCmd).action(guarded(async (person, opts, cmd) => {
        applyFromJson(cmd, opts, NOTIFY_FROM_JSON);
        requireFlags(cmd, opts, ["title", "body"]);
...
const OLETUS_ADMIN_YRITYS_ID_TYPE_ID = createRequire(import.meta.url)("@ibetoni/constants").PERSON_SETTING_TYPE_IDS.OLETUS_ADMIN_YRITYS_ID;
/**
...
export async function runPersonGetDefaultCompany(client, personId) {
    const rows = await client.get(`/api/person/setting/get/${personId}/${OLETUS_ADMIN_YRITYS_ID_TYPE_ID}`);
