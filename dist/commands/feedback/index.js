 ...
 */
export const OWNER_GATE_KINDS = ["owner", "owner-decision", "owner-action"];
/**
 ...
 */
export const AUTO_CLOSE_GATE_KINDS = ["deploy", "legal"];
/**
...
const TRUNCATE_HINT = "description/resolution/errorText over 200 chars show head+tail (middle elided) so an appended update is never cut off; ib dev feedback get <id> for full text";
/**
 ...
 */
const HELD_IGNORED_HINT = "⚠ --held was IGNORED by this backend (rows came back that no live lease holds) — " +
    "the filter is deploy-gated and predates the backend serving --endpoint. These results are " +
 ...
 */
function heldFilterIgnored(held, rows, me) {
    if (!held || !rows.length)
        return false;
 ...
 */
function gateValueMatches(gated, gateKind) {
    if (gated === "any")
        return true;
    if (gated === "owner-any")
        return OWNER_GATE_KINDS.includes(String(gateKind));
 ...
 */
export function parseDurationMs(raw, flag) {
    const m = DURATION_RE.exec(raw.trim());
    if (!m) {
        failWith(`${flag} must be a duration with a unit — <n>m (minutes), <n>h (hours) or <n>d (days), e.g. ${flag} 12h`, 4);
    }
    const n = Number(m[1]);
    const unit = m[2];
 ...
 */
const wouldSend = (method, path, body) => ({ dryRun: true, wouldSend: { method, path, body } });
 ...
 */
export async function runFeedbackImport(client, entries, flags = {}) {
    const results = new Array(entries.length);
    let next = 0;
    await Promise.all(Array.from({ length: Math.min(FEEDBACK_IMPORT_CONCURRENCY, entries.length) }, async () => {
...
function resolveStatuses(opts) {
    ...
    if (opts.status) {
        ...
        if (wantedAll.length) {
            ...
            failWith(`--status: unknown value(s) ${wantedAll.join(", ")} — must be one of: ${STATUSES.join(", ")}. For EVERY status pass the --all flag; ${STATUS_ALL_SENTINELS.join("/")} are not status values`, 4);
        }
 ...
 */
function downgradeDerivedMine(items, idSource, me) {
    if (idSource !== "derived" || !items.some((r) => r.claimState === "mine"))
        return items;
    const downgraded = items.map((r) => r.claimState === "mine" ? { ...r, claimState: "held" } : r);
...
export async function runFeedbackList(client, opts) {
    ...
    if (gatedKind)
        assertEnum(gatedKind, GATED_FILTERS, "--gated");
    ...
    if (!clientSideFilter && (!statuses || statuses.length <= 1)) {
        ...
        const filtered = merged.filter((r) => {
            if (opts.ungated && r.gateKind != null)
                return false;
...
export async function runFeedbackLink(client, id, relatedId, input) {
    ...
    const body = { relatedFeedbackId: relatedId, relationType: input.type };
    if (input.note !== undefined)
        body.note = input.note;
    if (input.dryRun)
        return wouldSend("POST", `/api/feedback/${id}/relations`, body);
...
export async function runFeedbackUnlink(client, id, relatedId, input) {
    if (input.dryRun) {
 ...
 */
export async function runFeedbackCluster(client, id) {
    const data = await client.get(`/api/feedback/${id}/cluster`);
    const { by: me, source: idSource } = resolveClaim(undefined);
    const items = downgradeDerivedMine((data.rows ?? []).map((r) => ({ ...r, claimState: deriveClaimState(r, me) })), idSource, me);
    const env = listEnvelope(items);
    if (data.truncated)
        env.truncated = true;
    return env;
 ...
 */
function readChangelogLinks(row) {
 ...
 */
function warnAlreadyLinked(row) {
    const links = readChangelogLinks(row);
    if (!links.length)
        return;
    const id = row.feedbackId ?? "?";
    const rendered = links.map((l) => `cl#${l.changelogId} (${l.role})`).join(", ");
    warnNote(`[ib] note: fb#${id} already carries ${links.length} changelog link${links.length > 1 ? "s" : ""} — ${rendered}. ` +
 ...
 */
function warnPartlyShipped(items) {
 ...
 * stderr, never stdout: the JSON data contract is untouched.
 */
function warnFreshRows(items, minAgeGiven) {
    if (minAgeGiven)
        return;
    const now = Date.now();
    ...
    warnNote(`[ib] note: ${fresh.length} of ${active.length} active rows were filed in the last ${FRESH_ROW_WINDOW_H}h ` +
 ...
 */
function buildAck(row, keys, cappedField) {
    const ack = {};
    for (const k of keys) {
        if (k in row)
            ack[k] = row[k];
    }
    if (cappedField in row)
        ack[cappedField] = truncateField(row[cappedField]).value;
    if ("warning" in row)
...
function compactUpdateAck(row) {
    return buildAck(row, ["feedbackId", "scope", "kind", "severity", "complexity", "gateKind", "gateRef", "gateUntil", "updatedAt"], "description");
}
/**
 ...
 */
function derivedIdentityNote(by) {
    return (`the label "${by}" was DERIVED from user@host, not chosen — nothing set $IB_CLAIM_ID, --by, or an MCP session id. ` +
 ...
 */
export async function runFeedbackGateClear(client, input) {
    if (!input.kind)
        failWith("--kind is required (deploy | legal)", 4);
    ...
    return client.post("/api/feedback/gates/clear", body);
}
/**
...
export function registerFeedbackCommands(parent, getClient, opts = {}) {
    ...
    f.command("import")
        ...
        writeJson(await runFeedbackImport(await getClient(), arr, opts));
    }));
    f.command("list")
        ...
        .option("--min-age <duration>", `Exclude rows filed within this duration (<n>m|<n>h|<n>d) — the rows still likely owned by the session that filed them. ${FRESH_ROW_WINDOW_H}h is the suggested floor for a batch-fix slice`)
    ...
    f.command("link <id> <relatedId>")
        ...
        .option("--note <text>")
        .option("--dry-run")
        .action(guarded(async (idStr, relatedStr, opts) => {
    ...
    f.command("reopen <id> [note]")
        ...
        applyFromJson(cmd, opts, REOPEN_FROM_JSON);
    ...
    f.command("gate-clear")
        ...
        .option("--dry-run")
        .action(jsonAction(getClient, (client, opts) => runFeedbackGateClear(client, opts)));
}
