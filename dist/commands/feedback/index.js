...
function warnPartlyShipped(items) {
    ...
    warnNote(`[ib] note: ${linked.length} of ${active.length} un-closed rows already carry changelog links (${named}${rest > 0 ? `, +${rest} more` : ""}) — ` +
...
export function registerFeedbackCommands(parent, getClient, opts = {}) {
    ...
    f.command("cluster <id>").action(guarded(async (idStr) => {
