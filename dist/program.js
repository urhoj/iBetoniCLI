...
import { buildUnknownCommandEnvelope, buildUnknownOptionEnvelope, buildExcessArgumentsEnvelope, dateFlagSuggestion, excessPositionals, commandPath, specForPath, optionNamesIn, optionsHoldingFlagName } from "./output/unknownCommand.js";
...
export async function buildProgram(argv) {
    ...
    async function clientFrom(inv) {
        ...
        if (!ctx.client) {
            ...
            failWith(`Not logged in${endpoint ? ` at ${endpoint}` : ""}. Run \`ib auth login${endpoint ? ` --endpoint ${endpoint}` : ""}\` first.`, 2, await otherSessionsHint(endpoint));
        }
        return ctx.client;
    }
    ...
    async function otherSessionsHint(endpoint) {
        if (!endpoint)
            return undefined;
        let hosts = [];
        try {
