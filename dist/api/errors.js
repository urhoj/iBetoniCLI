 ...
 */
export function hintForError(err, specErrors, writeFlags) {
 ...
 */
export function hintDetailForError(err, specErrors, writeFlags) {
    ...
    if (httpRow?.remedy)
        return { hint: httpRow.remedy, source: "spec" };
    const clientRow = err.statusCode === 0 ? matchClientRow(err, specErrors) : undefined;
    if (clientRow?.remedy)
        return { hint: clientRow.remedy, source: "spec" };
...
function genericHint(err, writeFlags) {
    if (err.exitCode === 7) {
