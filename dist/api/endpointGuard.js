...
export function assertWritableEndpoint(endpoint, forceProd) {
    ...
    if (isLocal || forceProd)
        return;
    throw new CliError(`Refused: '${endpoint}' is a shared-cache (deployed) endpoint. Cache writes there affect production (prod and staging share Redis DB 3). Re-run with --force-prod to override, or use a local endpoint.`, 0, null, 3);
}
