 ...
 */
import { hiddenDomainsAtTier } from "../tier.js";
...
export const GLOSSARY = [
    ...
    },
    {
        ...
        definition: "A customer company you deliver to (`ib customer …`). NOTE: 'asiakas' is also the word for a tenant company itself — `ib customer` = companies you deliver to, `ib company` = the tenant you act as (see 'company (tenant)').",
    },
    {
    ...
    },
    {
        ...
        definition: "A concrete batching plant — sijainti type Betoniasema. plant = factory = tehdas = (betoni)asema, used interchangeably ('Kivikon asemalla' / 'Kivikon tehtaalla' = the Kivikko betoniasema). Plants belong to SUPPLIER companies (Rudus, Lujabetoni, …), not your tenant — list with `ib sijainti plants` (alias: tehtaat), resolve by name with `--search <name>`.",
    },
    {
    ...
    },
    {
        ...
        definition: "The active company your token acts on — the multi-tenancy boundary (`ib company …`). Same Finnish word as customer — see 'asiakas / customer'.",
    },
    {
    ...
    {
        term: "viesti / message",
        definition: "Conversational chat threads between a customer and a provider/operator, tied to a context (Jerry tarjous now, keikka later) (`ib message chat …`). Distinct from daily grid notes and the announcement board.",
    },
    {
        term: "ilmoitustaulu / announcement board",
        definition: "Company-wide dated notices (title + body + priority info/warning/urgent) every member sees until they expire — a one-to-many broadcast, tenant-scoped (`ib message board …`, alias `ib message ilmoitustaulu`). Distinct from chat threads and daily grid notes.",
    },
    {
        term: "varikko / depot",
    ...
    {
        term: "validate",
        definition: "Validation profiles run as `ib validate`: company profiles (jerry = BetoniJerry provider readiness, betoni = betoni.online customer setup) and the person profile onboarding (is one employee ready to work — työsuhteessa, role, contact, role-required modules). Pass/fail/skip checklists with Finnish details.",
    },
    {
        term: "legal",
...
export function glossaryForTier(tier) {
    ...
    return GLOSSARY.filter((g) => ![...hidden].some((d) => new RegExp(`\\b${d}\\b`, "i").test(g.term)));
}
/**
