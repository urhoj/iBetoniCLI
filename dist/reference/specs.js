...
const BASE_COMMAND_SPECS = [
    ...
    // ─── auth (6) ────────────────────────────────────────────────────────────
    {
        ...
        errors: [
            { exit: 2, meaning: "OAuth flow failed", remedy: "retry; check network / browser" },
            {
                ...
                remedy: "the server's error is surfaced immediately without opening the browser (no 5-min callback hang) — fix the server-side cause (e.g. OAuth client registration / Redis) or the endpoint/network, then retry",
            },
    ...
    // ─── keikka (6) ──────────────────────────────────────────────────────────
    {
        ...
        outputShape: "ListEnvelope<{ keikkaId, pvm, asiakasId, tyomaaId, vehicleId, tila, m3, time }> & { range: { from, to } } (the interpreted date window, echoed so an empty result is verifiably scoped). On an empty result the envelope also carries a `hint` explaining the count:0 (permitted-but-empty vs how to widen).",
    ...
    },
    {
        ...
        description: "Decline a request as a provider WITHOUT making an offer (POST /api/pumppuRequests/:id/decline). Your company bows out; --reason is stored and shown to the customer (who is emailed + pushed that a provider passed). The request leaves your Avoimet tab (moves to Päättyneet). Blocked (409) if you already have an active offer — use `ib jerry offer withdraw` instead. Idempotent. Requires provider role + --reason.",
        ...
        writeFlags: true,
        outputShape: "{ success: true, declined: true, hasOtherProviders } (or { …, alreadyDeclined: true }) · { dryRun: true, wouldDecline: { pumppuRequestId, reason } } on --dry-run",
        errors: [
    ...
    },
    {
        ...
        description: "Reverse a prior decline as a provider (POST /api/pumppuRequests/:id/undecline). The request returns to your Avoimet tab and is offerable again. Idempotent (no-op success if you had not declined). No customer notification. Requires provider role + --reason.",
        permissions: ["provider company (isPumppuToimittaja)"],
        args: [{ name: "requestId", type: "number", description: "pumppuRequestId you previously declined" }],
        flags: [{ name: "reason", type: "string", description: "Audit-log reason (X-Action-Reason); REQUIRED" }],
        writeFlags: true,
        outputShape: "{ success: true, undeclined: boolean } · { dryRun: true, wouldUndecline: { pumppuRequestId } } on --dry-run",
        errors: [
            apiErr(404, "Request not found / not a recipient", "verify the requestId and that it was sent to your company"),
            apiErr(403, "Not a provider", "switch to a provider company (company switch)"),
            ...COMMON_AUTH_ERRORS,
    ...
    },
    {
        ...
        description: "One request's full detail — date, customer, placing operator, worksite, m³, status, offer count, accepted/best price, plus the send-time recipient list with per-company fanout state (notified/viewed/declined/hasOffer) (GET /api/admin/jerry-requests/:id). For the offers use `ib jerry admin request offers`. System-admin only.",
        permissions: ["isSystemAdmin"],
        tier: "developer",
        args: [{ name: "requestId", type: "number", description: "pumppuRequestId" }],
        flags: [],
        outputShape: "{ pumppuRequestId, status, createdAt, sentAt, expiresAt, totalM3, kayttokohde, customerAsiakasId, customerNimi, operatorName, osoite, offerCount, acceptedPriceCents, bestPriceCents, recipients: [{ asiakasId, asiakasNimi, notifiedAt, viewedAt, declinedAt, declineReason, hasOffer }] }",
        errors: [
    ...
    },
    {
        ...
        description: "Searched addresses (BetoniJerry coverage-checks) aggregated by place, with searchCount and a covered vs no_supply split — the signal for where to expand provider coverage (GET /api/admin/jerry-searches). Filters: --from/--to (createdAt), --deliverable (covered | no_supply), --q (address substring), --limit. System-admin only.",
        permissions: ["isSystemAdmin"],
        tier: "developer",
        flags: [
            { name: "from", type: "string", description: "createdAt from (YYYY-MM-DD/today/yesterday)" },
        ...
        outputShape: "ListEnvelope<{ label, osoite, formattedAddress, placeId, lat, lng, searchCount, noSupplyCount, notGeocodedCount, deliverableEver, maxProviderCount, nearestVarikkoKm, lastSearchedAt }>",
        errors: [apiErr(403, "Not a system admin", "use a system-admin token"), ...COMMON_AUTH_ERRORS],
        examples: [
    ...
    },
    {
        ...
        description: "BetoniJerry conversion funnel over a date window (GET /api/admin/jerry-searches/funnel): top-of-funnel coverage checks, wizard step 1..5 by distinct session, claimed count, and the outcome breakdown of claimed requests by status. System-admin only.",
        permissions: ["isSystemAdmin"],
        tier: "developer",
        flags: [
            { name: "from", type: "string", description: "createdAt from (YYYY-MM-DD/today/yesterday)" },
        ...
        outputShape: "{ coverageChecks: { total, deliverable, notDeliverable, notGeocoded }, wizard: { sessions, step1, step2, step3, step4, step5, claimed }, outcomes: { [status]: count } }",
        errors: [apiErr(403, "Not a system admin", "use a system-admin token"), ...COMMON_AUTH_ERRORS],
        ...
        flags: [
            {
                name: "full",
                type: "boolean",
                description: "Accepted for cross-command consistency; get always returns the full row (no-op).",
            },
        ],
    ...
    },
    {
        ...
        outputShape: "A compact ack { feedbackId, status, updatedAt, resolution } (resolution capped at 200 chars; the full row with --full). A note-only call that leaves the row open/reviewed adds hint naming the closing statuses. With --dry-run: { dryRun:true, wouldSend:{ method, path, body } }.",
        errors: [
    ...
    // logs so the task stays due. ALL developer-gated server-side this phase.
    {
        ...
        description: "List recurring operator tasks, most-overdue first (nextDueAt ASC). Developer-only. Default scope: active tasks; --due narrows to tasks due NOW (nextDueAt <= now); --inactive includes deactivated ones. The daily AI runner sweeps `ib task list --due --executor ai --agent claude`.",
        permissions: ["isSystemAdmin or isDeveloper"],
        tier: "developer",
        flags: [
        ...
        outputShape: "{ items: TaskRow[], nextCursor: null, count, truncated? } — TaskRow = { taskId, title, instructions, skillRef, executor, recommendedAgent, assigneePersonId, asiakasId, cadenceUnit, cadenceCount, nextDueAt, lastDoneAt, active, feedbackId, ... }",
        errors: [
    ...
    },
    {
        ...
        flags: [],
        outputShape: "The full task row { taskId, title, instructions, skillRef, executor, recommendedAgent, assigneePersonId, asiakasId, cadenceUnit, cadenceCount, nextDueAt, lastDoneAt, active, feedbackId, createdAt, updatedAt, ... }",
        errors: [
            apiErr(403, "Permission denied", "requires a developer token"),
    ...
    {
        command: "ib task add",
        description: "Create a recurring task (developer-only; a write). executor=human tasks surface in the morning report for a person to complete; executor=ai tasks are picked up by the daily runner when --skill names a workspace skill (--agent claude) — skill-less or hermes tasks wait and surface in the morning report. Default first due = immediately; the first completion sets the rhythm (nextDueAt = completion time + cadence, rolling, not day-anchored).",
        permissions: ["isSystemAdmin or isDeveloper"],
        tier: "developer",
        writeFlags: true,
        mutates: true,
    ...
    },
    {
        ...
        description: "Complete a task: append a done-log row and advance nextDueAt = now + cadence (rolling). --skipped also advances; --failed only logs — the task STAYS due (the runner's failure path). AI completions pass --agent so the log distinguishes human vs AI. Developer-only; a write.",
        permissions: ["isSystemAdmin or isDeveloper"],
        tier: "developer",
        writeFlags: true,
        mutates: true,
        ...
        outputShape: "{ logId, task } (task = the updated row; nextDueAt advanced unless --failed). With --dry-run: { dryRun:true, wouldComplete:{ taskId, outcome, advancesNextDue } }.",
        errors: [
    ...
    {
        command: "ib task set",
        description: 'Partial update of a recurring task (developer-only; a write). Omit a flag to KEEP the current value; pass "" to CLEAR a text field (--instructions/--skill/--agent). --deactivate soft-retires the task (history kept); --activate restores it. --next-due overrides the due date without logging a completion.',
        permissions: ["isSystemAdmin or isDeveloper"],
        tier: "developer",
        writeFlags: true,
        mutates: true,
    ...
    },
    {
        ...
        outputShape: "{ items: LogRow[], nextCursor: null, count, truncated? } — LogRow = { logId, taskId, doneAt, donePersonId, agent, outcome, notes }",
        errors: [
            apiErr(403, "Permission denied", "requires a developer token"),
            apiErr(404, "Not found", "check the id via `ib task list --inactive`"),
            apiErr(500, "Backend error", "retry with --verbose"),
