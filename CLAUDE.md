...
The explicit selection is `CommandSpec.prettyColumns` — worth setting on any list wider than ~8 columns, especially when the triage-relevant columns sit at the END of the row (the leftmost-fits fallback is deliberately dumb, and would hide exactly those). It is wired through `applySpecErrors` alongside the error rows, and the global `--columns <csv>` overrides it. Purely presentational: the JSON contract does not move. **The global flag is `--columns`, not `--fields`** — a root option is recognized anywhere in argv and would shadow the per-command `--fields` that `customer list` / `ohje list` already own for real payload projection.
