...
`list` **defaults to the active bucket** (status `open` + `reviewed`) — closed items (`applied`/`dismissed`) are hidden unless you pass `--all` (every status) or name them via `--status`. `--all` / `--unresolved` / `--status` are mutually exclusive (exit 4). It caps each row's `description`/`resolution`/`errorText` at 200 chars by default (full text via `ib dev feedback get <id>` or `--full`); when anything was cut the envelope carries a `hint`. The default, `--unresolved` (= `--status open,reviewed`), and a comma-separated `--status` fan out to one GET per status, merged newest-first client-side; `--all` and a single `--status` are one server-filtered GET; `list` also takes a server-side `--severity <s|none>` filter. `resolve` returns a compact ack `{ feedbackId, status, updatedAt, resolution }` by default (`--full` for the whole row). `ib dev feedback count` is `GET /api/feedback/stats` — aggregated SERVER-side over the whole table, returning seven keys (`total, byStatus, byKind, byScope, bySeverity, unestimated, ungraded`); it falls back to the old client-side rollup (four keys, capped at the 200-row page) only against a backend that predates that route. `ib dev feedback lint` is `GET /api/feedback/lint` — the completeness audit (incomplete rows, not just open ones). The whole group is **deploy-gated on `puminet5api`**, not client-side only.
